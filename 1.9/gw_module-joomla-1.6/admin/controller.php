...
class GlosswordController extends JController
{
	...
	function admin_cph()
	{
		...
		$ar_cfg['db_host'] = $jo_app->getCfg( 'host' );
		$ar_cfg['db_user'] = $jo_app->getCfg( 'user' );
		$ar_cfg['db_pass'] = $jo_app->getCfg( 'password' );
		if ( $ar_cfg['db_name'] == $jo_cfg->getValue( 'db' ) )
		{
			/* Display error */
			/* *REMOVED* */
			/* Catch only mysqli */
			if ( $jo_app->getCfg( 'dbtype' ) == 'mysqli' )
			{
	 ...
	 */
	function user_sync($ar, $id_usergroup = false )
	{
		$jo_db =& JFactory::getDBO();
		$jo_user = & JFactory::getUser();
		...
		$query = 'SELECT id `id_user` FROM #__users WHERE id != "1" ORDER BY registerDate DESC LIMIT 1000';
		$jo_db->setQuery( $query );
		$ar_sql = $jo_db->loadAssocList();
