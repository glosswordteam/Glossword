...
class HTML_glossword
{
	function showform($ar)
	{
		...
		for (; list($k, $v) = each($ar); )
		{
			...
			if ( $k == 'path_core_abs' )
			{
				$str_append = '/public_html/joomla/components/com_glossword/core';
			}
			/* 16 Jan 2010 - Disable changing database settings */
			/* Read from the system */
			if ($k == 'db_driver' && !$v) { continue; $v = $jo_app->getCfg('dbtype'); }
			if ($k == 'db_host' && !$v) { continue; $v = $jo_app->getCfg('host'); }
			if ($k == 'db_name' && !$v) { $v = $jo_app->getCfg('db'); }
			if ($k == 'db_user' && !$v) { continue; $v = $jo_app->getCfg('user'); }
			if ($k == 'db_pass' && !$v) { continue; $v = $jo_app->getCfg('password'); }
			...
			if ($k == 'path_temp_abs' && !$v) { $v = str_replace( '\\', '/', $jo_app->getCfg('tmp_path') ); }
			if ($k == 'path_temp_web' && !$v) { $v = end($ar_path_temp); }
			if ($k == 'path_core_abs' && !$v) { $v = str_replace( '\\', '/', JPATH_ROOT.'/components/com_glossword/core'); }

			if ( $k == 'path_temp_web' )
			{
				$str_append = JText::sprintf( 'LJADM0005', $ar['server_dir'].'/<strong>'.$v.'</strong>/gw_joomla/header.png');
			}
			$str_append = ( $str_append ? '<div style="float:left;width:80%">'.$str_append.'</div>' : '' );
