...
$ar_req_fields = ['title', 'id_topic', 'uid', 'tablename', 'lang', 'cfg', 'visualtheme'];
...
if ($this->gw_this['vars']['post'] == '') {
    ...
    if (isset($arPost['date_modifS'])) /* Fix date */ {
        ...
        if (!preg_match("/[0-9]{6}/", $arPost['date_createdS'])) {
            $arPost['date_createdS'] = '000000';
        }
    }
    if (isset($arPost['date_modifS'])) /* Fix date */ {
        $arPost['date_modifS'] = str_replace(":", "", $arPost['date_modifS']);
        if (!preg_match("/[0-9]{6}/", $arPost['date_modifS'])) {
            $arPost['date_modifS'] = '000000';
        }
    }
    /* Fix on/off options */
    $arOnOff = [
        ...
        'is_sens_chr',
        'is_sens_dia',
        'is_show_date_modified',
        'is_show_authors',
        'is_abbr_long',
        'is_show_term_suggest',
        'is_show_term_report',
        'is_show_page_refresh',
        'is_show_page_send',
        'is_show_add_to_favorites',
        'is_show_add_to_search',
        'is_show_printversion',
    ];
...
#$this->sys['isDebugQ'] = 1;
        $arDictNewSettings = [
            ...
            'ar_filter_stopwords'  => isset($arPost['ar_filter_stopwords']) ? array_flip($arPost['ar_filter_stopwords']) : [],
            'ar_filter_specials'   => isset($arPost['ar_filter_specials']) ? array_flip($arPost['ar_filter_specials']) : [],
        ];
        foreach ($arOnOff as $k => $v) {
