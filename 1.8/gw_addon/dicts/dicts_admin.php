...
class gw_addon_dicts_admin extends gw_addon
{
     ...
     */
    public function get_form_dict($vars, $runtime = 0, $ar_broken = [], $ar_req = [])
    {
        ...
        /* Search length */
        $arNums = [0 => $this->oL->m('1038'), 1, 2, 3, 4, 5];
        /* Tree of topics */
        ...
        if ($this->gw_this['vars'][GW_ACTION] == GW_A_ADD) {
            ...
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field('select', 'arPost[lang]', $vars['lang'], '0', array_merge([$this->oL->m('1270')], $this->gw_this['vars']['ar_languages'])) . '</td>' .
                '</tr>';
            $str_form .= '<tr>' .
                '<td class="' . $v_class_1 . '">' . $this->oL->m('visual_theme') . ':' . $ar_req_msg['visualtheme'] . '</td>' .
                '<td class="' . $v_class_2 . '">' . $oForm->field('select', 'arPost[visualtheme]', $vars['visualtheme'], 0, array_merge([$this->oL->m('1270')], $this->gw_this['ar_themes_select'])) . '</td>' .
                '</tr>';
            $str_form .= '</tbody></table>';
            $str_form .= '</fieldset>';
            ...
            $str_form .= '<tr>' .
                ...
                '<td class="td2">' . $oForm->field('select', 'arPost[lang]', $vars['lang'], 0, array_merge([$this->oL->m('1270')], $this->gw_this['vars']['ar_languages'])) . '</td>' .
                '</tr>';

            /* Visual themes */
            $str_form .= '<tr>' .
                '<td class="td1">' . $this->oL->m('visual_theme') . ':' . $ar_req_msg['visualtheme'] . '</td>' .
                '<td class="td2 gray">' . $oForm->field('select', 'arPost[visualtheme]', $vars['visualtheme'], 0, array_merge([$this->oL->m('1270')], $this->gw_this['ar_themes_select']));
...
#			$str_form .= '<fieldset class="admform"><legend class="xq">&#160;</legend>';
            ...
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field('select', 'arPost[recent_terms_display]', $vars['recent_terms_display'], 0, [
                    $this->oL->m('is_0'),
                    $this->oL->m('1286'),
                    $this->oL->m('1287'),
                ]) . '</td>' .
            ...
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field('select', 'arPost[recent_terms_sorting]', $vars['recent_terms_sorting'], 0, [
                    $this->oL->m('1272'),
                    $this->oL->m('1273'),
                ]) . '</td>' .
                '</tr>';
