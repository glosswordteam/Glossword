...
$mode = (string) $this->gw_this['vars']['mode'];
$ar_queries = [];
...
/* Check permissions. */
$page_owner_id = (int) $ar_parsed['id_user'];
...
switch ($mode) {
    ...
    case 'off':
    case 'on':
        if ($this->_set_page_active($page_id, $mode)) {
            return;
        }
        break;
}
...
if ($this->gw_this['vars']['post'] == '') {
    ...
    if (!empty($this->gw_this['vars']['remove'])) {
        ...
        foreach ($ar_parsed['page'] as $ar_page) {
            if ($ar_page['id_lang'] == $this->gw_this['vars']['locale_name']) {
                $str_pagename = '<div>'
        ...
        $o_form_confirm->setField('hidden', GW_TARGET_ID, $page_id);
        $o_form_confirm->setField('hidden', 'w1', $this->gw_this['vars']['w1']);
        $o_form_confirm->setField('hidden', 'w2', $this->gw_this['vars']['w2']);
    ...
    if (isset($ar_pre['page']) && is_array($ar_pre['page'])) {
        foreach ($ar_pre['page'] as $page_offset => $ar_v) {
            ...
            $q_page_phrase = [
                ...
                'page_keywords'  => isset($ar_v['page_keywords']) ? gw_fix_input_to_db($ar_v['page_keywords']) : '',
