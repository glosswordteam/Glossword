...
#		$fieldname = $this->component;
        ...
        for (; list($elK, $arV) = each($vars['page']);) {
            ...
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field(
                    'input',
                    'arPre[page][' . $elK . '][page_title]',
                    gw_fix_db_to_field($arV['page_title'])
                ) . '</td>' .
                '</tr>';
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field(
                    'textarea',
                    'arPre[page][' . $elK . '][page_descr]',
                    gw_fix_db_to_field($arV['page_descr']),
                    $this->oFunc->getFormHeight($arV['page_descr'])
                ) . '</td>' .
                '</tr>';
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field(
                    'textarea',
                    'arPre[page][' . $elK . '][page_content]',
                    gw_fix_db_to_field($arV['page_content']),
                    $this->oFunc->getFormHeight($arV['page_content'])
                ) . '</td>' .
                '</tr>';
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field(
                    'textarea',
                    'arPre[page][' . $elK . '][page_keywords]',
                    gw_fix_db_to_field($arV['page_keywords']),
                    $this->oFunc->getFormHeight($arV['page_keywords'])
                ) . '</td>' .
                '</tr>';
            $str_form .= '<tr>' .
                ...
                '<td class="' . $v_class_2 . '">' . $oForm->field(
                    'select',
                    'arPre[page][' . $elK . '][id_lang]',
                    $arV['id_lang'],
                    0,
                    $this->gw_this['vars']['ar_languages'],
                    '',
                    'input50'
                ) . '</td>' .
                '</tr>';
        ...
        $str_form .= '<tr>' .
            ...
            '<td class="' . $v_class_2 . '">' . $this->sys['server_dir'] . '/' . $this->sys['path_temporary'] . '/t/' . $this->sys['visualtheme'] . '/<br />' . $oForm->field(
                'input',
                'arPre[page_icon]',
                gw_fix_db_to_field($vars['page_icon'])
            ) . '</td>' .
            '</tr>';
        $str_form .= '<tr>' .
            '<td class="' . $v_class_1 . '">' . $this->oL->m('1073') . ':</td>' .
            '<td class="' . $v_class_2 . '">' . $this->sys['server_dir'] . '/' . GW_A_CUSTOMPAGE . '/<br />' . $oForm->field(
                'input',
                'arPre[page_uri]',
                gw_fix_db_to_field($vars['page_uri'])
            ) . '</td>' .
            '</tr>';
        ...
        $str_form .= '<tr>' .
            ...
            '<td class="' . $v_class_2 . '">&lt;?' . 'php<br />' . $oForm->field(
                'textarea',
                'arPre[page_php_1]',
                gw_fix_db_to_field(
                    $vars['page_php_1']
                ),
                $this->oFunc->getFormHeight(
                    $vars['page_php_1']
                )
            ) . '<br />?&gt;</td>' .
            '</tr>';
...
#		$str_form .= $oForm->field('hidden', 'arPost[id_action_old]', $vars['id_action_old']);
    ...
    public function get_form_export($vars)
    {
        ...
        $str_form .= '<tr>' .
            ...
            '<td class="td2">' . '<label for="' . $oForm->text_field2id('arPost[is_as_file]') . '">' . $this->oL->m(
                '1299'
            ) . '</label></td>' .
            '</tr>';
