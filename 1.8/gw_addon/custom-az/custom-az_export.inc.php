...
$ar_profile = $this->oDb->sqlExec($this->oSqlQ->getQ('get-custom_az-profile', $this->gw_this['vars'][GW_TARGET_ID]), $this->component);
$this->ar_profile = isset($ar_profile[0]) ? $ar_profile[0] : [];
/* Profile not found */
if (empty($this->ar_profile)) {
    $is_error = 1;
}
if ($this->gw_this['vars'][GW_TARGET_ID] == 1) {
...
/* Filename */
$this->filename = $this->gw_this['vars'][GW_TARGET] . '_' . $this->gw_this['vars'][GW_TARGET_ID] . '_' . preg_replace('#[^a-zA-Z0-9-]#', '', $this->ar_profile['profile_name']) . '.xml';
...
if ($this->gw_this['vars']['post'] == '') {
    ...
    /* Get letters */
    $arSql = $this->oDb->sqlExec($this->oSqlQ->getQ('get-custom_az-letters', $this->gw_this['vars'][GW_TARGET_ID]), $this->component);
