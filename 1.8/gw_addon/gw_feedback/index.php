...
class gw_addon_feedback extends gw_addon
{
    ...
    public function get_form_feedback($vars, $runtime = 0, $ar_broken = [], $ar_req = [])
    {
        $str_hidden = '';
        $str_form = '';
        $v_td1_width = '25%';
    ...
    public function alpha()
    {
        ...
        $fields = ['message', 'term', 'defn', 'title', 'url', 'subject', 'email1'];
        foreach ($fields as $f) {
        ...
        switch ($this->gw_this['vars']['uid']) {
            case 'newterm':
                $ar_req_fields = ['term', 'defn', 'captcha'];
                if (isset($this->gw_this['vars']['q']) && strlen($this->gw_this['vars']['q'])) {
                    $vars['term'] = strip_tags($this->gw_this['vars']['q']);
                }
                break;
            case 'report':
                $ar_req_fields = ['defn', 'captcha'];
                break;
            case 'sendpage':
                $ar_req_fields = ['name', 'email', 'email1', 'subject', 'message', 'captcha'];
