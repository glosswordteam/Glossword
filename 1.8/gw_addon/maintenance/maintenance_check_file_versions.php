...
/*
	    Maintenance task: create RSS with list of recently modified files.
*/
/* */
require_once($sys['path_addon'] . '/class.gw_addon.php');
...
{
    /** @var string */
    public $addon_name = 'check_file_versions';
    ...
    {
        // Use minimal init for API/maintenance context
        $this->init_m();
    }

    /**
     * Build RSS with list of recently modified files.
     *
     * @return void
     */
    protected function _gw_create_rss()
    {
        clearstatcache();
        ...
        foreach ($ar_fm as $date_modified => $filename) {
            ...
            // TODO: Move URL to config, avoid hardcoded host
            $file_url = 'https://github.com/glosswordteam/Glossword/commits/stand-alone/' . $filename;
        ...
        $filename_output = $this->sys['path_temporary'] . '/versions.xml';
        $this->oFunc->file_put_contents($filename_output, $str_rss, 'w');
    }

    /**
     * Run task with probability.
     *
     * @return void
     */
