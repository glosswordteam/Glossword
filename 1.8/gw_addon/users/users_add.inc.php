...
$ar_req_fields = ['login', 'pass_new', 'pass_confirm'];

/* correct unknown settings */
$ar_user_settings = [
    'is_show_contact' => 1,
    'locale_name'     => $this->gw_this['vars']['locale_name'],
    'visualtheme'     => 'gw_brand',
    'location'        => '',
    'avatar_img'      => '',
    'is_use_avatar'   => 0,
    'is_htmled'       => '1',
...
foreach ($ar_user_settings as $k => $v) {
    if (!isset($arSql['user_settings'][$k])) {
        $arSql['user_settings'][$k] = $v;
    }
}
/* Not submitted */
if ($this->gw_this['vars']['post'] == '') {
    ...
    /* Fix on/off options */
    $arIsV = ['is_show_contact', 'is_send_notice'];
...
#$this->sys['isDebugQ'] = 1;
        ...
        $arPost['user_settings']['dictionaries'] = isset($arPost['dictionaries']) ? $arPost['dictionaries'] : [];

        /* Selected permissions */
        $arPost['is_permissions'] = isset($arPost['is_permissions']) ? $arPost['is_permissions'] : [];
        foreach ($arPost['is_permissions'] as $k => $v) {
