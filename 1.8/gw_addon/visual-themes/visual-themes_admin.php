...
#			$arV['settings_value'] = str_replace(array('{', '}'), array('{%', '%}'), $arV['settings_value']);
            ...
            if (is_numeric($this->gw_this['vars']['w1']) && ($this->gw_this['vars']['w1'] <= 2)) {
                ...
                if (preg_match("/^is_/", $arV['settings_key'])) {
                    /* create checkbox */
                    $str_form .= '<tr>' .
                        '<td class="' . $v_class_1 . '"' . $bg_tag . '><label for="' . $oForm->text_field2id(
                            'arPost[' . $arV['settings_key'] . ']'
                        ) . '">' . $arV['settings_key'] . ':</label></td>' .
                ...
                $this->oHtml->setTag(
                    'a',
                    'onclick',
                    'return confirm(\'' . $this->oL->m(
                        '3_remove'
                    ) . ': &quot;' . $arV['settings_key'] . '&quot;. ' . $this->oL->m(
                        '9_remove'
                    ) . '\' )'
                );
