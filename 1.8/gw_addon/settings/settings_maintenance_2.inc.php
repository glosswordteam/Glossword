...
function gw_html_contents()
{
    ...
    $arStatus[] = [$oL->m('1010'), sprintf('<strong>%s</strong>', number_format($arWordlist['Rows'], 0, '', ' '))];
    $arStatus[] = [$oL->m('1011'), sprintf('<strong class="red">%s</strong>', number_format($intKeyUnused, 0, '', ' '))];
    $arStatus[] = [$oL->m('1012'), sprintf('<strong class="green">%s</strong>', number_format(($arWordlist['Rows'] - $intKeyUnused), 0, '', ' '))];
    $intKbWordlist = ($arWordlist['Data_free'] + $arWordlist['Data_length'] + $arWordlist['Index_length']);
    $arStatus[] = [$oL->m('1013'), sprintf('<strong>%s</strong>', number_format($intKbWordlist, 0, '', ' '))];
    if ($intKeyUnused > 0) {
        $arStatus[] = [
            $oL->m('1014'),
            sprintf('<strong class="red">%s</strong>',
                number_format((($intKbWordlist / $arWordlist['Rows']) * $intKeyUnused), 0, '', ' ')),
        ];
        $arStatus[] = [
            $oL->m('1015'),
            sprintf('<strong class="green">%s</strong>',
        ...
        if ($is == '') {
            $arStatus[] = ['&#160;'];
            $arStatus[] = [
                sprintf('<strong class="xw">%s</strong>', $oL->m('1016')),
                sprintf('<span class="actions-third"><a href="%s">%s</a></span>',
    ...
    $str .= $oL->m('1009');
    $str .= html_array_to_table_multi($arStatus, 0, ['65%', '35%']);
    $str .= '</div>';
    return $str;
}
...
function gw_optimize_keywords()
{
    ...
    if (!empty($arKeyId)) {
        ...
        if ($oDb->sqlExec($sql, '', 0)) {
            $arStatus[] = [sprintf('<strong class="red">%s</strong>', $oL->m('1018')), ''];
        }
    }
