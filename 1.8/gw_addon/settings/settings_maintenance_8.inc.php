...
function gw_show_form()
{
    ...
    $arStatus[] = [
        sprintf('<strong class="xw">%s</strong>', $oL->m('9_remove')),
        sprintf(
...
function gw_backup()
{
    ...
    switch ($gw_this['vars']['w2']) {
        ...
        case 3:
            $gw_cnt_dicts = 0;
            $arTables = [$sys['tbl_prefix'] . 'map_user_to_dict', $sys['tbl_prefix'] . 'map_user_to_term'];
            break;
        case 4:
            $gw_cnt_dicts = 0;
            $arTables = [$sys['tbl_prefix'] . 'theme_settings', $sys['tbl_prefix'] . 'theme', $sys['tbl_prefix'] . 'theme_group'];
            break;
        case 5:
            $gw_cnt_dicts = 0;
            $arTables = [$sys['tbl_prefix'] . 'history_terms'];
            break;
        case 6:
            $gw_cnt_dicts = 0;
            $arTables = [$sys['tbl_prefix'] . 'wordlist'];
            break;
        case 7:
            $gw_cnt_dicts = 0;
            $arTables = [$sys['tbl_prefix'] . 'wordmap'];
            break;
    }
    ++$gw_this['vars']['w2'];
    /* */
    if ($gw_this['vars']['w2'] < 8) {
        ...
        $arStatus[] = ['', $oL->m('2_success')];
    }
    /* Display the number of dictionaries */
    if ($gw_this['vars']['w2'] == 2 || $gw_this['vars']['w2'] == 3) {
        $arStatus[] = ['', $oL->m('1335') . ': <strong>' . ($gw_cnt_dicts) . '</strong>'];
    }
    /* */
    foreach ($arTables as $k => $v) {
        $arStatus[] = [$v, gw_export_sqltable($v)];
    }
    return $arStatus;
}
