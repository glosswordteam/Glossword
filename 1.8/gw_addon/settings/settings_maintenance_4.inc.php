...
function gw_get_sql_cache()
{
    ...
    if (($gw_this['vars']['isConfirm'] == '2') && $oDb->sqlExec(sprintf('DELETE FROM `%s`', $sys['tbl_prefix'] . 'search_results'))) {
        $arStatus[] = ['<span class="green">' . $oL->m('2_success') . '</span>'];
    }
    ...
    $arStatus[] = [$oL->m('1020'), $oFunc->number_format($int_size, 0, $oL->languagelist(LOCALE_LANG_RULES))];
    $arStatus[] = [$oL->m('1021'), $oFunc->number_format($arTableInfo['Rows'], 0, $oL->languagelist(LOCALE_LANG_RULES))];

    /* Link to confirm */
    if (($int_size > 1) && ($gw_this['vars']['isConfirm'] != 2)) {
        $arStatus[] = [
            sprintf('<strong class="xw">%s</strong>', $oL->m('1023')),
...
function gw_get_file_cache()
{
    ...
    $arStatus[] = [$oL->m('1022'), $oFunc->number_format($ar_tree['bytes'], 0, $oL->languagelist(LOCALE_LANG_RULES))];
    $arStatus[] = [$oL->m('1021'), $oFunc->number_format($ar_tree['files'], 0, $oL->languagelist(LOCALE_LANG_RULES))];
    /* Link to confirm */
