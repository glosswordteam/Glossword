...
function gw_show_form($vars, $runtime = 0, $arBroken = [], $arReq = [])
{
    ...
    foreach ((is_array($vars) ? $vars : []) as $key => $val) {
        ...
        if (isset($arReq[$key])) {
            $arReqMsg[$key] = '&#160;<span class="red"><strong>*</strong></span>';
        }
        if (isset($arBroken[$key])) {
            $arBrokenMsg[$key] = ' <span class="red"><strong>' . $oL->m('reason_9') . '</strong></span><br/>';
        }
    }
    ##
    ## ----------------------------------------------------
...
function gw_do_task($id_old, $id_new, $is_visible = 1)
{
    ...
    /* Run queries */
    if ($sys['isDebugQ']) {
        $arStatus[] = ['', '<ul class="gwsql"><li>' . implode(';</li><li>', $arQ) . ';</li></ul>'];
    } else {
        /* */
