...
class gw_addon_abbr_admin extends gw_addon
{
    ...
    public function __construct()
    {
        ...
        $this->oL->applyCustomPhrases(
            'addon_' . $this->addon_name,
            $this->gw_this['vars'][GW_LANG_I] . '-' . $this->gw_this['vars']['lang_enc']
        );
        $this->ar_groups = [
            '1' => $this->oL->m('speech'),
            '2' => $this->oL->m('science'),
    ...
    public function _get_nav()
    {
        ...
        foreach ($arSql as $k => $arV) {
            ...
            if ($arV['id_lang'] != $this->gw_this['vars']['w1'] && ($this->gw_this['vars'][GW_ACTION] == GW_A_EDIT)) {
                $ar_languages[$arV['id_lang']] = $this->oHtml->a(
                    $this->oUrlBuilder->build_admin_url(GW_A_EDIT, $this->addon_name, [
                        'w1' => $arV['id_lang'],
                        GW_TARGET_ID => $this->gw_this['vars'][GW_TARGET_ID],
                    ]),
                    $str_lang
                );
            } else {
                $ar_languages[$arV['id_lang']] = $this->oHtml->a(
                    $this->oUrlBuilder->build_admin_url(GW_A_BROWSE, $this->addon_name, ['w1' => $arV['id_lang'], 'w2' => $this->gw_this['vars']['w2']]),
                    $str_lang
                );
            }
        }
        ...
        foreach ($this->ar_groups as $k => $v) {
            $ar_topics[] = $this->oHtml->a(
                $this->oUrlBuilder->build_admin_url(GW_A_BROWSE, $this->addon_name, ['w1' => $this->gw_this['vars']['w1'], 'w2' => $k]),
                $v
            );
        }
        ...
        return sprintf(
            '<p class="xt gray">%s: %s</p><p class="xt gray">%s: %s</p>',
            $this->oL->m('lang'),
            implode(' | ', $ar_languages),
            $this->oL->m('topic'),
            implode(' | ', $ar_topics)
        );
    }
    ...
    /* */
    public function get_form_abbr($vars, $runtime = 0, $ar_broken = [], $ar_req = [])
    {
        $str_hidden = '';
        $str_form = '';
        $v_class_1 = 'td1';
        ...
        $str_form .= $oForm->field('hidden', GW_TARGET_ID, $vars['id_abbr_phrase']);
    ...
    public function browse()
    {
        ...
        foreach ($arSql as $k => $arV) {
            ...
            $strR .= $this->oHtml->a($href_edit, $arV['abbr_short'], '', '', $this->oL->m('3_edit'));

            $strR .= '&#160;</td></tr></tbody></table></td>';
            $strR .= '<td class="xu">';

            $strR .= $this->oHtml->a($href_edit, $arV['abbr_long'], '', '', $this->oL->m('3_edit'));

            $strR .= '</td>';
            $strR .= '<td class="actions-third" style="text-align:center">';

            $strR .= $this->oHtml->a($href_edit, $this->oL->m('3_edit'));
            ...
            $this->oHtml->setTag(
                'a',
                'onclick',
                'return confirm(\'' . $this->oL->m('3_remove') . ': &quot;' . htmlspecialchars($arV['abbr_short'] . ' ' . $arV['abbr_long']) . '&quot;. ' . $this->oL->m('9_remove') . '\' )'
            );

            $strR .= $this->oHtml->a($href_remove, $this->oL->m('3_remove'));
            ...
            $strR .= $arV['is_active']
                ? $this->oHtml->a($href_off, '<span class="green">' . $this->oL->m('is_1') . '</span>')
                : $this->oHtml->a($href_on, '<span class="red">' . $this->oL->m('is_0') . '</span>', $this->oL->m('1057'));
            $strR .= '</td>';
        ...
        $strR .= '</tbody></table>';
        $strR .= PHP_EOL . '<script type="text/javascript">/*<![CDATA[*/';
    ...
    {
        global $strR;
        $ar_req_fields = ['abbr_short', 'abbr_long', 'id_group', 'id_lang'];
        ...
        if ($this->gw_this['vars']['post'] == '') {
            /* Get Abbr settings */
            $arSql = $this->oDb->sqlExec($this->oSqlQ->getQ('get-abbr-by-id', $this->gw_this['vars'][GW_TARGET_ID], $this->gw_this['vars']['w1']), $this->addon_name);
    ...
    public function add()
    {
        ...
        if ($this->gw_this['vars']['post'] == '') {
            ...
            if (empty($ar_broken)) {
                ...
                $this->str .= postQuery(
                    $ar_query,
                    $this->oUrlBuilder->build_admin_url(GW_A_ADD, $this->addon_name, ['arPost' => ['id_lang' => $id_lang]]),
                    $this->sys['isDebugQ'],
                    $this->sys['isPause']
                );
            } else {
    ...
    public function remove()
    {
        ...
        if (!$this->gw_this['vars']['isConfirm']) /* if not confirmed */ {
            ...
            $oConfirm->setField('hidden', GW_TARGET_ID, $this->gw_this['vars'][GW_TARGET_ID]);
    ...
    public function alpha()
    {
        ...
        if (is_array($this->gw_this['vars']['arControl'])) {
            ...
            $this->gw_this['vars'][GW_TARGET_ID] = $this->gw_this['vars']['arPost']['id_abbr'];
        }
