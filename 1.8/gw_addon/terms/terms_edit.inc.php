...
if ($this->gw_this['vars']['w1'] == 'viewhistory') {
    ...
    $arCurrent = isset($arCurrent[0]) ? $arCurrent[0] : ['id' => 0];
    /* */
...
#$this->sys['isDebugQ'] = 1;
    ...
    /* Replace User ID */
    $arQ[] = gw_sql_update(['user_id' => $arH['id_user']], $this->sys['tbl_prefix'] . 'map_user_to_term', 'term_id = ' . $arTermParam['tid'] . ' AND dict_id = ' . $id_dict);
    /* Replace Term */
