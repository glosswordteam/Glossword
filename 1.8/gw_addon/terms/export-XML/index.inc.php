 ...
 *
 */
function getFormXml($vars, $runtime = 0, $arBroken = [], $arReq = [])
{
    ...
    foreach ((is_array($vars) ? $vars : []) as $key => $val) {
        ...
        if (isset($arReq[$key])) {
            $arReqMsg[$key] = ' <span style="color:#E30"><strong>*</strong></span>';
        }
        if (isset($arBroken[$key])) {
            $arBrokenMsg[$key] = ' <span class="' . $trClass . '" style="color:#E30"><strong>' . $oL->m('reason_9') . '</strong></span>';
        }
    } // end of while
    ##
    ## ----------------------------------------------------
    ...
    $arSplit = ['100' => '100', '500' => '500', '1000' => '1000', '2500' => '2500', '5000' => '5000'];
    $strForm .= '
	  		<tr>
			<td></td>
        ...
        for ($i = 0; $i < $cntFiles; $i++) {
            ...
            foreach ($arSql as $k => $v) {
                ...
                if ((($arPost['td_mode'] == 'td') || ($arPost['td_mode'] == 'd')) && ($v['defn'] != '')) {
                    if ($vars['is_cmptblty_old']) {
                        $v['defn'] = str_replace(['><![CDATA[', ']]><'], ['> ', ' <'], $v['defn']);
                    }
                    $strQ .= trim($v['defn']);
                }
                $strQ .= '</line>' . CRLF;
