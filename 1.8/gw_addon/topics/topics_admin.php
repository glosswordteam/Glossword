...
class gw_addon_topic_admin extends gw_addon
{
    ...
    /* */
    public function get_form_topic($vars, $runtime = 0, $ar_broken = [], $ar_req = [])
    {
        $str_hidden = '';
        $str_form = '';
        $v_class_1 = 'td1';
        ...
        foreach ($vars['topic'] as $elK => $arV) {
            $arV['topic_title'] = str_replace(['{', '}'], ['{%', '%}'], $arV['topic_title']);
            $arV['topic_descr'] = str_replace(['{', '}'], ['{%', '%}'], $arV['topic_descr']);
            $tmp['strBtnRemove'] = '';
...
#		$this->sys['isDebugQ'] = 1;
        ...
        if (isset($ar[$this->gw_this['vars']['tid']]['ch'])) {
            ...
            foreach ((is_array($arKeys) ? $arKeys : []) as $k => $v) {
