...
if ($gw_this['vars'][GW_TARGET]
    ...
    if (!preg_match("/&d=/", $udecoded)) {
        $udecoded = url_decrypt(2, $u);
    }
    if (!preg_match("/&d=/", $udecoded)) {
        $udecoded = url_decrypt(1, $u);
    }
    if (!preg_match("/&d=/", $udecoded)) {
        $udecoded = url_decrypt(0, $u);
    }
    $u = $udecoded;
    if ($u != '') {
...
// --------------------------------------------------------
$gw_this['vars']['funcnames'][GW_T_TERM] = isset($gw_this['vars']['funcnames'][GW_T_TERM]) ? $gw_this['vars']['funcnames'][GW_T_TERM] : [];
$gw_this['vars']['funcnames'][GW_T_DICT] = isset($gw_this['vars']['funcnames'][GW_T_DICT]) ? $gw_this['vars']['funcnames'][GW_T_DICT] : [];
...
/* Uppercase / lowercase */
$oCase = new gwv_casemap([1, 2, 3, 4, 5, 6, 7, 8], [1]);
...
if (!is_array($gw_this['vars'][GW_A_SEARCH])) {
    $gw_this['vars'][GW_A_SEARCH] = [];
}
if (!isset($gw_this['vars']['srch']['by'])) {
    $gw_this['vars']['srch']['by'] = 'd';
} // search by dictionary
if (!isset($gw_this['vars']['srch']['in'])) {
    $gw_this['vars']['srch']['in'] = -1;
}  // search in all elements
if (!isset($gw_this['vars']['srch']['adv'])) {
    $gw_this['vars']['srch']['adv'] = 'all';
} // search all words

// Set switcher for HTML
$arTplVars['srch'][] = ['v:chk_srch_strict' => '']; // search 'this dictionary only'
...
$arTplVars['srch'][] = ['v:chk_srch_by_topic' => ''];
$arTplVars['srch'][] = ['v:chk_srch_by_dict' => ''];
...
if (isset($gw_this['vars']['srch']['adv']) && $gw_this['vars']['srch']['adv'] == 'all') {
    $arTplVars['srch'][] = ['v:chk_srch_all' => ' checked="checked"'];
} elseif (isset($gw_this['vars']['srch']['adv']) && $gw_this['vars']['srch']['adv'] == 'exact') {
    $arTplVars['srch'][] = ['v:chk_srch_exact' => ' checked="checked"'];
} elseif (isset($gw_this['vars']['srch']['adv']) && $gw_this['vars']['srch']['adv'] == 'any') {
    $arTplVars['srch'][] = ['v:chk_srch_any' => ' checked="checked"'];
} else // default
{
    $arTplVars['srch'][] = ['v:chk_srch_all' => ' checked="checked"'];
}
...
if (isset($gw_this['vars']['srch']['by']) && $gw_this['vars']['srch']['by'] == 'd') {
    ...
    if (isset($gw_this['vars']['srch']['by']) && $gw_this['vars']['srch']['by'] == 'tp') {
        // Search by topics
        $arTplVars['srch'][] = ['v:chk_srch_by_topic' => ' checked="checked"'];
        // Get topic's tree ID
...
{
    $arTplVars['srch'][] = ['v:chk_srch_strict' => ' checked="checked"'];
}
/* --------------------------------------------------------
