...
class gw_mini_site
{
    ...
    public function init()
    {
        ...
        $this->a(
            ...
            isset($_SERVER['HTTP_ACCEPT_ENCODING']) ? $_SERVER['HTTP_ACCEPT_ENCODING'] : (isset($_SERVER['HTTP_TE']) ? $_SERVER['HTTP_TE'] : '')
        );
        /* Get remote IP string */
        $this->a(
            'REMOTE_ADDR',
            (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '') . (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && $_SERVER['HTTP_X_FORWARDED_FOR'] != 'unknown' ? ' FW: ' . $_SERVER['HTTP_X_FORWARDED_FOR'] : '') . (isset($_SERVER['HTTP_CLIENT_IP']) ? ' CLIENT_IP: ' . $_SERVER['HTTP_CLIENT_IP'] : '') . (isset($_SERVER['HTTP_VIA']) ? ' VIA: ' . $_SERVER['HTTP_VIA'] : '')
        );
        ...
        switch ($this->gv['arv']) {
            ...
            case 'js':
                ...
                break;
            default:
                /* HTML-templates */ $this->oTpl = $this->_init_html_tpl();
                /* XMLReader */
            ...
            for (reset($arV1['value']); list($k2, $arV2) = each($arV1['value']);) {
                ...
                switch ($arV2['tag']) {
                    ...
                    case 'entry':
                        ...
                        break;
                    default:
                        /* page_php_1, page_php_2 */ $q1[$arV2['tag']] = $arV2['value'];
                        break;
                }
            }
        ...
        for (reset($arData['style'][0]['value']['group']); list($k1, $arV1) = each($arData['style'][0]['value']['group']);) {
            ...
            for (reset($arV1['value']); list($k2, $arV2) = each($arV1['value']);) {
                for (reset($arV2); list($k3, $arV3) = each($arV2);) {
                    switch ($id_group) {
                        case 'settings';
                            /* Compatibility with old database */ $arV3['attributes']['key'] = str_replace('_', '\_', $arV3['attributes']['key']);
    ...
    public function page_header()
    {
        ...
        switch ($this->gv['arv']) {
            ...
            case 'js':
                break;
            default:
                /* Check for locked installation */ if (file_exists($this->g('file_lock'))) {
        ...
        switch ($this->gv['arv']) {
            ...
            default:
                ...
                if ($this->g('is_debug_time')) {
                    ...
                    $this->oTpl->addVal(
                        'v:debug',
                        '<div class="debugwindow">' . 'Total: <strong>' . sprintf("%1.3f", $time_php + $db_time) . '</strong> - ' . 'PHP: <strong>' . sprintf("%1.3f", $time_php) . '</strong> (init: ' . $this->oTimer->_('init') . ' + proc: ' . $this->oTimer->_('proc') . ') - SQL: <strong>' . $db_time . '</strong> (Queries: <strong>' . $query_count . '</strong>) - Memory, bytes: ' . $this->oTkit->number_format(memory_get_usage() - $this->g('debug_memory_s')) . ' - ' . $this->V->path_views . '/' . $this->cur_function . '.php' . $str_debug_sql . '</div>'
                    );
                }
                /* */
