...
$ar_menu = [
    '.999999' =>
        $this->oHtmlTags->a_href(
            [$this->g('file_index'), 'arg[target]' => 'install', 'arg[il]' => $this->gv['il']],
            [],
            $this->oTkit->_(20002)
        ),
];
...
foreach (glob($this->g('path_views') . '/update_1*.php') as $filename) {
    /* Parse filenames */
    $filename = str_replace([$this->g('path_views') . '/', '.php'], '', $filename);
    ...
    $ar_menu[sprintf(".%02d%02d%02d", $version_to1, $version_to2, $version_to3)] = $this->oHtmlTags->a_href(
        [$this->g('file_index'), 'arg[target]' => 'update', 'arg[action]' => $action, 'arg[il]' => $this->gv['il']],
        [],
...
$ar_menu[] = $this->oHtmlTags->a_href(
    [$this->g('file_index'), 'arg[target]' => 'uninstall', 'arg[il]' => $this->gv['il']],
    [],
    $this->oTkit->_(20004)
);
/* Construct menu */
$this->oTpl->addVal(
    'v:select_action',
    '<ul class="select-action"><li>' . implode('</li><li>', $ar_menu) . '</li></ul>'
);
