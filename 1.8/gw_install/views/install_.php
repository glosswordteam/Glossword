...
switch ($this->gv['step']) {
    case 1:
        ...
        if (file_exists($this->g('path_locale') . '/' . $license_file)) {
            ...
            $this->oTpl->addVal(
                'v:text_before',
                '<form accept-charset="utf-8" action="' . $this->g(
                    'file_index'
                ) . '" enctype="application/x-www-form-urlencoded" id="form-install" method="post">' . '<p>' . $this->oTkit->_(
                    20023
                ) . '</p><p>' . $this->oTkit->_(20024) . '</p>'
            );

            $this->oTpl->addVal(
                'v:text_after',
                '<div class="center">' . '<p>' . $this->oTkit->_(
                    20025
                ) . '</p>' . '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step(
                    $this->gv['step']
                ) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                    20026
                ) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(
                    20027
                ) . '</a>' . '</div></form>'
            );
        }
    ...
    case 2:
        ...
        $this->oTpl->addVal(
            'v:text_after',
            '<div class="center">' . '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step(
                $this->gv['step']
            ) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[post]" value="1" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                20007
            ) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(
                10002
            ) . '</a>' . '</div></form>'
...
function INSTALL_select_db_name(id)
{
    ...
    case 3:
            ...
            if ($this->gv['arp']['use_db'] == 'new') {
                ...
                if (@mysqli_select_db($db_conn, $this->gv['arp']['db_name_new'])) {
                    ...
                    $this->oTpl->addVal(
                        ...
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(
                                20052,
                                '<strong>' . $this->gv['arp']['db_name_new'] . '</strong>'
                ...
                if (@mysqli_select_db($db_conn, $this->gv['arp']['db_name_existent'])) {
                    ...
                    $this->oTpl->addVal(
                        'v:note_afterpost',
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(20054, $this->gv['arp']['db_name_existent']),
                            true
                        )
                    );
                } else {
                    ...
                    $this->oTpl->addVal(
                        ...
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(
                                20049,
                                '<strong>' . $this->gv['arp']['db_name_existent'] . '</strong>'
        ...
        $this->oTpl->addVal(
            ...
            $this->oTkit->_(
                10012
            ) . ': <strong>' . $ar_info['product'] . '</strong> ' . $ar_info['version']
        );
        ...
        $this->oTpl->addVal(
            ...
            '<form accept-charset="utf-8" action="' . $this->g(
                'file_index'
            ) . '" enctype="application/x-www-form-urlencoded" id="form-install" method="post">'
        );
        $this->oTpl->addVal(
            'v:text_inside',
            '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step(
                $this->gv['step']
            ) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[db_settings]" value="' . base64_encode(
                serialize($this->gv['arp'])
            ) . '" />
		'
        );
        if ($this->oChecker->GetChecked()) {
            $this->oTpl->addVal(
                'v:text_after',
                '<div class="center">' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                    20007
                ) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(
                    10002
                ) . '</a>' . '</div></form>'
            ...
            $this->oTpl->addVal(
                ...
                $this->oHtml->get_note_afterpost(
                    $this->oTkit->_(10048),
                    false
                ) . '<br /><br /><div class="center">' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                    20007
                ) . '</a>' . ' <a href="#" class="submitcancel" onclick="window.location.reload();return false">' . $this->oTkit->_(
                    10049
                ) . '</a>' . '</div></form>'
            );
        }
    ...
    case 4:
        ...
        $this->oTpl->addVal(
            'v:text_after',
            '<div class="center">' . '<input type="hidden" name="arg[target]" value="install" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step(
                $this->gv['step']
            ) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[post]" value="1" />' . '<input type="hidden" name="arp[db_settings]" value="' . ($this->gv['arp']['db_settings']) . '" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                20007
            ) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(
                10002
            ) . '</a>' . '</div></form>'
...
'
    ...
    case 5:
        ...
        $this->oDb->update(
            'settings',
            ['settings_val' => $this->gv['arp']['admin_email']],
            ['settings_key' => 'y\_email']
        );
        $this->oDb->update(
            'settings',
            ['settings_val' => $this->gv['arp']['admin_email']],
            ['settings_key' => 'site\_email']
        );
        $this->oDb->update(
            'settings',
            ['settings_val' => $this->gv['arp']['admin_email']],
            ['settings_key' => 'site\_email\_from']
        );
        ...
        */
        ...
        $str_oo = str_replace(
            '{v:server_url}',
            strip_tags(
                $this->gv['arp']['server_proto'] . $this->gv['arp']['server_host'] . $this->gv['arp']['server_dir']
            ),
            $str_oo
        );
        ...
        $this->oTpl->addVal(
            'v:text_inside',
            '<input type="hidden" name="arg[target]" value="install" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step(
                $this->gv['step']
            ) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp" value="' . base64_encode(
                serialize($this->gv['arp'])
            ) . '" />'
        );
    ...
    case 6:
        ...
        if (isset($this->gv['arp']['db_settings']['is_preinstall'])) {
            ...
            $this->oDb->update(
                'dict',
                ['date_created' => $this->g('time_gmt') - 60, 'date_modified' => $this->g('time_gmt')],
                ['id' => '1']
            );
            $this->oDb->update(
                'history_terms',
        ...
        $this->oDb->insert(
            'custom_az_profiles',
            ['id_profile' => '1', 'is_active' => '1', 'profile_name' => '! UTF-8 Order']
        );
        $ar_items = [];
    ...
    case 7:
        ...
        $this->gv['arp']['server_url'] = $this->gv['arp']['server_proto'] . $this->gv['arp']['server_host'] . rtrim(
                $this->gv['arp']['server_dir'],
                '/'
            );
        $this->oTpl->addVal(
            'v:text_inside',
...
'
        ...
        $this->oFunc->file_put_contents(
            $this->g('file_lock'),
            'When this file is here, installation script is disabled.',
            'w'
        );
