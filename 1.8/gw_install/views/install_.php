...
switch ($this->gv['step']) {
    case 1:
        ...
        if (file_exists($this->g('path_locale') . '/' . $license_file)) {
            ...
            $this->oTpl->addVal(
                'v:text_before',
                '<form accept-charset="utf-8" action="' . $this->g('file_index') . '" enctype="application/x-www-form-urlencoded" id="form-install" method="post">' . '<p>' . $this->oTkit->_(20023) . '</p><p>' . $this->oTkit->_(20024) . '</p>'
            );

            $this->oTpl->addVal(
                'v:text_after',
                '<div class="center">' . '<p>' . $this->oTkit->_(20025) . '</p>' . '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step($this->gv['step']) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(20026) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(20027) . '</a>' . '</div></form>'
            );
        }
        break;
    case 2:
        ...
        $this->oTpl->addVal(
            'v:text_before',
            '<form accept-charset="utf-8" action="' . $this->g('file_index') . '" enctype="application/x-www-form-urlencoded" id="form-install" method="post">' . '<p>' . $this->oTkit->_(20029) . '</p>'
        );
        ...
        $this->oTpl->addVal(
            'v:text_after',
            '<div class="center">' . '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step($this->gv['step']) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[post]" value="1" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(20007) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(10002) . '</a>' . '</div></form>'
        );
...
<label for="arp-db-type-"><em>' . $this->oTkit->_(20030) . '&#160;<strong class="state-warning">*</strong></em>
	<select id="arp-db-type-" class="inp w25" name="arp[db_type]"><option value="mysqli" selected="selected">MySQLi</option></select>
</label>
...
function INSTALL_select_db_name(id)
{
    ...
    case 3:
        ...
        $db_name = $this->gv['arp']['db_name_existent'] ? $this->gv['arp']['db_name_existent'] : $this->gv['arp']['db_name_new'];
        ...
        if ($db_conn) {
            if ($this->gv['arp']['use_db'] == 'new') {
                /* Create new database */
                if (@mysqli_select_db($db_conn, $this->gv['arp']['db_name_new'])) {
                    /* Database already exists */
                    $this->oTpl->addVal(
                        'v:note_afterpost',
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(20052, '<strong>' . $this->gv['arp']['db_name_new'] . '</strong>') . '<br />' . $this->oTkit->_(20053),
                            false
                        )
                    );
                    $this->ar_broken['db_name_new'] = true;
                ...
                $sql = "CREATE DATABASE `" . mysqli_real_escape_string($db_conn, $this->gv['arp']['db_name_new']) . "`";
                ...
                if (!mysqli_query($db_conn, $sql)) {
                    ...
                    $this->oTpl->addVal(
                        'v:note_afterpost',
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(20050, '<strong>' . $this->gv['arp']['db_name_new'] . '</strong>') . '<br />' . $this->oTkit->_(20051),
                            false
                        )
                    );
                    $this->ar_broken['db_name_new'] = true;
                ...
                if (@mysqli_select_db($db_conn, $this->gv['arp']['db_name_existent'])) {
                    ...
                    $this->oTpl->addVal(
                        'v:note_afterpost',
                        $this->oHtml->get_note_afterpost(
                            $this->oTkit->_(20049, '<strong>' . $this->gv['arp']['db_name_existent'] . '</strong>') . '<br />' . $this->oTkit->_(20048),
                            false
                        )
                    );
                    $this->ar_broken['db_name_existent'] = true;
            ...
            $this->oTpl->addVal(
                'v:note_afterpost',
                $this->oHtml->get_note_afterpost(
                    $this->oTkit->_(20047, '<strong>' . $this->gv['arp']['db_host'] . '</strong>', '<strong>' . $this->gv['arp']['db_user'] . '</strong>') . '<br />' . $this->oTkit->_(20048) . '<br />' . mysqli_connect_errno() . ' - ' . mysqli_connect_error(),
                    false
                )
            );
            $this->gv['step']--;
        ...
        /* Continue */
        $config = $this->oXml->get('reqcheck_glossword.xml');
        if (!is_array($config)) {
            $this->oTpl->addVal(
                'v:note_afterpost',
                $this->oHtml->get_note_afterpost(
                    $this->oTkit->_('Unable to read reqcheck_glossword.xml')
                )
            );
            $this->gv['step']--;
            $this->page_body();
        ...
        if ($result) {
            while ($arV = mysqli_fetch_assoc($result)) {
                $db_max_allowed_packet = (int)$arV['Value'];
            }
            mysqli_free_result($result);
        }
        $ar_results[] = [
        ...
        $this->oTpl->addVal(
            'v:text_inside',
            '<input type="hidden" name="arg[target]" value="' . $this->gv['target'] . '" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step($this->gv['step']) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[db_settings]" value="' . base64_encode(serialize($this->gv['arp'])) . '" />
		'
        );
        if ($this->oChecker->GetChecked()) {
            $this->oTpl->addVal(
                'v:text_after',
                '<div class="center">' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(20007) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(10002) . '</a>' . '</div></form>'
            );
        } else {
            $this->oTpl->addVal(
                'v:text_after',
                $this->oHtml->get_note_afterpost($this->oTkit->_(10048), false) . '<br /><br /><div class="center">' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(20007) . '</a>' . ' <a href="#" class="submitcancel" onclick="window.location.reload();return false">' . $this->oTkit->_(10049) . '</a>' . '</div></form>'
            );
        }
        break;
    case 4:
        ...
        $this->oTpl->addVal(
            'v:text_after',
            '<div class="center">' . '<input type="hidden" name="arg[target]" value="install" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step($this->gv['step']) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp[post]" value="1" />' . '<input type="hidden" name="arp[db_settings]" value="' . ($this->gv['arp']['db_settings']) . '" />' . ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(20007) . '</a>' . ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(10002) . '</a>' . '</div></form>'
        );
...
<label for="arp-server-dir-"' . $ar_class_incorrect['server_dir'] . '><em>' . $this->oTkit->_(20062) . '</em>
...
'
    ...
    case 5:
        ...
        $this->oDb->update('settings', ['settings_val' => $this->gv['arp']['admin_name']], ['settings_key' => 'y\_name']);
        $this->oDb->update('settings', ['settings_val' => $this->gv['arp']['admin_email']], ['settings_key' => 'y\_email']);
        $this->oDb->update('settings', ['settings_val' => $this->gv['arp']['admin_email']], ['settings_key' => 'site\_email']);
        $this->oDb->update('settings', ['settings_val' => $this->gv['arp']['admin_email']], ['settings_key' => 'site\_email\_from']);
        $this->oDb->update('settings', ['settings_val' => 'gw\_brand'], ['settings_key' => 'visualtheme']);
        $this->oDb->update('settings', ['settings_val' => $this->g('site_name')], ['settings_key' => 'site\_name']);
        $this->oDb->update('settings', ['settings_val' => $this->g('site_desc')], ['settings_key' => 'site\_desc']);
        ...
        */
        ...
        $this->oTpl->addVal(
            'v:text_inside',
            '<input type="hidden" name="arg[target]" value="install" />' . '<input type="hidden" name="arg[step]" value="' . $this->get_next_step($this->gv['step']) . '" />' . '<input type="hidden" name="arg[il]" value="' . $this->gv['il'] . '" />' . '<input type="hidden" name="arp" value="' . base64_encode(serialize($this->gv['arp'])) . '" />'
        );
    ...
    case 6:
        ...
        if (isset($this->gv['arp']['db_settings']['is_preinstall'])) {
            ...
            $this->oDb->update('dict', ['date_created' => $this->g('time_gmt') - 60, 'date_modified' => $this->g('time_gmt')], ['id' => '1']);
            $this->oDb->update('history_terms', ['date_created' => $this->g('time_gmt') - 60, 'date_modified' => $this->g('time_gmt')], ['id_dict' => '1']);
            $ar_results[] = [
                'tag'     => 'db',
        ...
        $this->oDb->insert('custom_az_profiles', ['id_profile' => '1', 'is_active' => '1', 'profile_name' => '! UTF-8 Order']);
        $ar_items = [];
    ...
    case 7:
        ...
        $this->gv['arp']['server_url'] = $this->gv['arp']['server_proto'] . $this->gv['arp']['server_host'] . rtrim($this->gv['arp']['server_dir'], '/');
        $this->oTpl->addVal(
            'v:text_inside',
            '
<label>
