...
switch ($this->gv['step']) {
    case 1:
        ...
        $this->oTpl->addVal(
            ...
            ' <a href="#" class="submitcancel" onclick="history.back(-1);return false">' . $this->oTkit->_(
                10016
            ) . '</a>' .
            ' <a href="#" class="submitok" onclick="document.forms[0].submit();return false">' . $this->oTkit->_(
                20004
            ) . '</a>' .
            '</div></form>'
        );
...
if (!empty($ar_status)) {
    $this->oTpl->addVal(
        'v:text_inside',
        '<ul class="select-action"><li>' . implode('</li><li>', $ar_status) . '</li></ul>'
    );
}
