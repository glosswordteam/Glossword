...
class gw_mini_site
{
    ...
    public $oTkit, $oHtml, $oTimer, $oFunc, $oChecker, $oHtmlTags, $oTpl;
    public $V, $gv, $ar_broken;
...
#		$o->oDb =& $this->oDb;
    ...
    public function import_topics_file($filename)
    {
        ...
        $ar_data = $this->oXml->get($filename);
    ...
    public function import_visual_themes_file($filename)
    {
        ...
        for (
            reset($arData['style'][0]['value']['group']); list($k1, $arV1) = each(
            $arData['style'][0]['value']['group']
        );
        ) {
    ...
    public function import_custom_az_file($filename)
    {
        ...
        /* */
        foreach ($arData['custom_az'][0]['value']['entry'] as $entry_index => $entry_data) {
            ...
            default:
                ...
                if (isset($this->oDb->queries)) {
                    ...
                    if ($this->g('is_show_debug_db')) {
                        ...
                        $str_debug_sql .= '<ol title="database"><li>' . implode(
                                '</li><li>',
                                $this->oDb->queries
                            ) . '</li></ol>';
                    }
                ...
                if ($this->g('is_debug_time')) {
                    ...
                    $this->oTpl->addVal(
                        'v:debug',
                        '<div class="debugwindow">' . 'Total: <strong>' . sprintf(
                            "%1.3f",
                            $time_php + $db_time
                        ) . '</strong> - ' . 'PHP: <strong>' . sprintf(
                            "%1.3f",
                            $time_php
                        ) . '</strong> (init: ' . $this->oTimer->_('init') . ' + proc: ' . $this->oTimer->_(
                            'proc'
                        ) . ') - SQL: <strong>' . $db_time . '</strong> (Queries: <strong>' . $query_count . '</strong>) - Memory, bytes: ' . $this->oTkit->number_format(
                            memory_get_usage() - $this->g('debug_memory_s')
                        ) . ' - ' . $this->V->path_views . '/' . $this->cur_function . '.php' . $str_debug_sql . '</div>'
                    );
                }
                /* */
