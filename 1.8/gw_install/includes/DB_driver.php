...
class CI_DB_driver
{
    ...
    public $db_debug_q    = false; /* Do not run queries */
    public $benchmark     = 0;
     ...
     * @return    void
     */
    public function initialize($create_db = false)
    {
        ...
        if (!$this->conn_id) {
            ci_log_message('error', 'Unable to connect to the database');
    ...
    public function query($sql, $binds = false, $return_object = true)
    {
        ...
        if ($this->db_debug_q) {
            $this->queries[] = '<b>-.-----</b> db_debug_q=on ' . htmlspecialchars(substr($sql, 0, 2048)) . '... <i>' . strlen($sql) . ' bytes</i>';
            return;
        }
...
#			include_once(BASEPATH.'database/drivers/'.$this->dbdriver.'/'.$this->dbdriver.'_result'.EXT);
    ...
    public function old_display_error($error = '', $swap = '', $native = false)
    {
        global $LANG;
        $LANG->load('db');
