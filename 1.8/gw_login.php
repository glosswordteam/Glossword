 ...
 * ----------------------------------------------------- */
$gw_this['vars'] = $oGlobals->register([
    GW_ACTION,
    GW_ID_DICT,
    'p',
    'r',
    ...
    'q',
    'gw_visualtheme',
    'gw_is_save_visualtheme',
    'gw_' . GW_LANG_I,
    'gw_is_save_' . GW_LANG_I,
]);
$gw_this['cookie'] = [];
...
$gw_this['vars']['is']['save_' . GW_LANG_I] = (isset($gw_this['cookie']['gw_is_save_' . GW_LANG_I]) && $gw_this['cookie']['gw_is_save_' . GW_LANG_I] == 1) ? 1 : 0;
...
$tmp['ar_req'] = ['user_name', 'user_pass'];
/* */
$gw_this['str_status'] = '';
...
$oSess->load_settings();
/* Get Session ID */
$id_sess = isset($gw_this['cookie'][GW_SID]) ? $gw_this['cookie'][GW_SID] : (isset($gw_this[GW_SID]) ? $gw_this[GW_SID] : 0);
/* */
$str_login = '';
$vars = [];
/* */
function gw_login_form($ar_vars, $ar_broken = [], $ar_req = [])
{
    ...
    if ($gw_this['vars'][GW_ACTION] == 'lostpass') {
        ...
        $oForm->strNotes = '<span class="xt">' . $oHtml->a(
        ...
        $strForm .= '<fieldset class="admform"><legend class="xq">&#160;</legend>';
        $strForm .= '<table class="gw2TableFieldset" width="100%"><tbody>';
        $strForm .= '<tr>' . '<td style="width:35%" class="' . $v_class_1 . '">' . $oL->m('y_email') . ':' . $ar_req_m['user_email'] . '</td>' . '<td class="' . $v_class_2 . '">' . $ar_broken_m['user_email'] . $oForm->field('input', 'arPost[user_email]', textcodetoform($ar_vars['user_email']), 64) . '</td>' . '</tr>';
        $strForm .= '</tbody></table>';
        $strForm .= '</fieldset>';
        ...
        /* "log in" page */
        $oForm->strNotes = '<span class="xt">' . $oHtml->a(
                $oSess->url_login . '?' . GW_TARGET . '=' . GW_T_USERS . '&' . GW_ACTION . '=lostpass',
        ...
        $strForm .= '<tr>' . '<td class="' . $v_class_1 . '"><label for="arPost_user_pass_">' . $oL->m('password') . ':' . $ar_req_m['user_pass'] . '</label></td>' . '<td class="' . $v_class_2 . '">' . $ar_broken_m['user_pass'] . $oForm->field('pass', 'arPost[user_pass]', textcodetoform($ar_vars['user_pass']), 16) . '</td>' . '</tr>';
        $oForm->setTag('select', 'class', 'input');
        $oForm->setTag('select', 'style', 'width:98%');
        $strForm .= '<tr>' . '<td class="' . $v_class_1 . '"><label for="arPost_locale_name_">' . $oL->m('lang') . ':</label></td>' . '<td class="' . $v_class_2 . '">' . $oForm->field('select', 'arPost[locale_name]', $ar_vars['locale_name'], 0, $gw_this['vars']['ar_languages']) . '</td>' . '</tr>';
        $strForm .= '</tbody></table>';
        $strForm .= '</fieldset>';
...
if ($gw_this['vars']['k']) {
    ...
    if (empty($arSql)) {
        ...
        $str_body = str_replace(
            ['{USERNAME}', '{SITENAME}', '{LOGIN_URL}', '{CONTACT_EMAIL}', '{LOGIN}', '{PASSWORD}', '{U_LOGIN}', '{U_PASSWORD}'],
            [
                $arSql['user_name'],
                strip_tags($sys['site_name']),
    ...
    if ($gw_this['vars'][GW_ACTION] == 'lostpass') {
        ...
        if ($gw_this['vars']['arPost']['user_name'] || $gw_this['vars']['arPost']['user_email']) {
            ...
            if ($gw_this['vars']['arPost']['user_name']) {
                ...
                if ($gw_this['vars']['arPost']['user_email']) {
