...
class gwForms {
    ...
    public function field($formtype = 'input', $formname = '', $value = '', $textareaheight = 2, $array = '', $autofocus = 0)
    {
        ...
        switch ($formtype)
        {
            ...
            case "input90":
                $str .= sprintf(
                    '<input name="%s" value="%s" style="width:60px" class="input" />',
                    htmlspecialchars((string)$formname, ENT_QUOTES, 'UTF-8'),
            ...
            case "select":
                ...
                $option_list = is_array($array) ? $array : [];
                $is_multiple = is_array($value);
                $selected_value = $is_multiple ? '' : (string) $value;
                $title_list = is_array($textareaheight) ? $textareaheight : [];
                ...
                foreach ($option_list as $option_value => $option_text) {
                    ...
                    } elseif ((string) $option_value === $selected_value) {
                        /* Single */
                        $selected_attr = ' selected="selected"';
                    }
                    ...
                    if (isset($title_list[$option_value])) {
                        $title_attr = ' title="' . gw_html_escape((string) $title_list[$option_value]) . '"';
                    }

                    $str .= PHP_EOL . "\t" .
                        '<option value="' . gw_html_escape((string) $option_value) . '"' .
                        $title_attr .
                        $selected_attr .
                        '>' . gw_html_escape((string) $option_text) . '</option>';
                }
                $str .= '</select>';
                break;
            default:
