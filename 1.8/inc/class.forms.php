...
class gwForms
{
    ...
    public function field($formtype = 'input', $formname = '', $value = '', $textareaheight = 2, $array = '', $autofocus = 0)
    {
        ...
        switch ($formtype) {
            ...
            case "checkbox":
                ...
                // sometimes value is not just `1' or `0'
                if ($value == 1)
                {
            ...
            case "select":
                ...
                foreach ((is_array($array) ? $array : []) as $k => $v) {
                    ...
                    $str .= sprintf(
                        CRLF . "\t" . '<option value="%s"%s%s>%s</option>',
    ...
    public function get_button($b)
    {
        switch ($b) {
            case 'submit':
                $this->cnt_submit++;
                return '<input id="submit' . $this->cnt_submit . '" accesskey="S" tabindex="1" onclick="' .
                        $this->onclickSubmit . '" type="submit" name="post" title="' .
                        $this->submitok . ' (CTRL+Enter)" value="' . $this->submitok . '" class="submitok" />';
                break;
            case 'cancel':
                $this->cnt_cancel++;
                return '<input id="cancel' . $this->cnt_cancel . '" onclick="' .
                        $this->onclickCancel . '" type="reset" value="' .
                        $this->submitcancel . '" class="submitcancel "/>';
                break;
            case 'delete':
                $this->cnt_delete++;
                return '<input id="delete' . $this->cnt_delete . '" onclick="document.getElementById(\'vbcontrol\').style.visibility=\'hidden\'" type="submit" name="' .
                        $this->submitdelname . '" value="' .
                        $this->submitdel . '" class="submitdel" />';
                break;
        }
    }
}
