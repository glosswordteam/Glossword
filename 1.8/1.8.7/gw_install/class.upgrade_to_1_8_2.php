<?php
class gw_setup_upgrade_to_1_8_2 extends gw_setup
{
	/* */
	function upgrade_to_1_8_2_step_1()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.1', '1.8.2') );
		...
		$this->str_before .= '<li><p>'.sprintf($this->oL->m('1251'), '1.8.1').'</p></li>';
		$this->str_before .= '</ol>';
	...
	/* */
	function upgrade_to_1_8_2_step_2()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.1', '1.8.2') );
...
#		$is_continue = ($is_continue && $this->post_queries($ar_q));
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up182_data.sql';
		$sql_q = implode('', file($sql_file));
		$sql_q = str_replace('{PREFIX}', $this->gv['arpost']['dbprefix'], $sql_q);
		$ar_q = explode("\r\n", $sql_q);
		unset($ar_q[sizeof($ar_q)-1]);
...
#		$ar_q = $this->import_templates_file($xml_file);
	...
	/* Update dictionaries */	
	function upgrade_to_1_8_2_step_4()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.1', '1.8.2') );
		...
		{
			$sql_file = $this->sys['path_install'].'/sql/glossword1_up182_dict_'.$this->sys['db_type'].'.sql';
			$ar_q_src = file($sql_file);
			$ar_q_src = array_map('trim', $ar_q_src);
			for (reset($arSql); list($kQ, $arV) = each($arSql);)
			{
				$ar_q = array();
				$this->ar_status[] = sprintf($this->oL->m('1248'), $arV['tablename']);
				for (reset($ar_q_src); list($k, $v) = each($ar_q_src);)
				{
					$ar_q[$k] = sprintf($v, $arV['tablename']);
				}
				$ar_q[] = sprintf("UPDATE `%s` SET `int_bytes` = LENGTH(defn)", $arV['tablename']);
				$ar_q[] = sprintf("OPTIMIZE TABLE `%s`", $arV['tablename']);
	...
	/* */	
	function upgrade_to_1_8_2_step_5()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.1', '1.8.2') );
