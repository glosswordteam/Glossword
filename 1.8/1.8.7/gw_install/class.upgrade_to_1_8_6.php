<?php
class gw_setup_upgrade_to_1_8_6 extends gw_setup
{
	/* */
	function upgrade_to_1_8_6_step_1()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.5', '1.8.6') );
	...
	/* */
	function upgrade_to_1_8_6_step_2()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.5', '1.8.6') );
...
#			$this->gv['arpost']['pass_new'] = $this->oFunc->text_make_uid(8, 0);
	...
	function upgrade_to_1_8_6_step_3()
	{
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up186_structure_'.$this->gv['arpost']['db_type'].'.sql';
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up186_data.sql';
		$sql_q = implode('', file($sql_file));
		$sql_q = str_replace('{PREFIX}', $this->gv['arpost']['dbprefix'], $sql_q);
		$ar_q = explode("\r\n", $sql_q);
		unset($ar_q[sizeof($ar_q)-1]);
		...
		$is_field_exists['id_custom_az'] = 0;
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!$is_field_exists['id_custom_az'])
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'dict` ADD `id_custom_az` MEDIUMINT(8) UNSIGNED NOT NULL DEFAULT \'1\' AFTER `id_user`');
		}
		$ar_q = $q1 = array();
		/* Topics */
		/* Custom pages */
	...
	/* Update dictionaries */
	function upgrade_to_1_8_6_step_4()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.5', '1.8.6') );
		...
		if ($cntDicts > 0)
		{
			...
			/* */
			$sql_file = $this->sys['path_install'].'/sql/glossword1_up186_dict_'.$this->sys['db_type'].'.sql';
			...
			foreach($arT as $arV)
			{
				 $is_field_type[$arV['Field']] = strtolower($arV['Type']);
			}
			if (isset($is_field_type['term_0']))
			{
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` DROP `term_0`');
			}
			if ($is_field_type['is_active'] != 'tinyint(1) unsigned')
			{
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` CHANGE `is_active` `is_active` TINYINT(1) UNSIGNED NOT NULL DEFAULT \'1\'');
				$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET `is_active` = "0" WHERE `is_active` = "1"');
				$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET `is_active` = "1" WHERE `is_active` = "2"');
			}
			if ($is_field_type['is_complete'] != 'tinyint(1) unsigned')
			{
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` CHANGE `is_complete` `is_complete` TINYINT(1) UNSIGNED NOT NULL DEFAULT \'1\'');
				$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET `is_complete` = "0" WHERE `is_complete` = "1"');
				$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET `is_complete` = "1" WHERE `is_complete` = "2"');
			}
			/* for each query */
	...
	/* */
	function upgrade_to_1_8_6_step_5()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.5', '1.8.6') );
