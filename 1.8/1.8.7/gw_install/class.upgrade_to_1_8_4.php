<?php
class gw_setup_upgrade_to_1_8_4 extends gw_setup
{
	/* */
	function upgrade_to_1_8_4_step_1()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.3', '1.8.4') );
		...
		$this->str_before .= '<li><p>'.sprintf($this->oL->m('1251'), '<b>1.8.3</b>').'</p></li>';
		$this->str_before .= '</ol>';
	...
	/* */
	function upgrade_to_1_8_4_step_2()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.3', '1.8.4') );
...
#			$this->gv['arpost']['pass_new'] = $this->oFunc->text_make_uid(8, 0);
	...
	/* */
	function upgrade_to_1_8_4_step_3()
	{
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up184_structure_'.$this->gv['arpost']['db_type'].'.sql';
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up184_data.sql';
		$sql_q = implode('', file($sql_file));
		$sql_q = str_replace('{PREFIX}', $this->gv['arpost']['dbprefix'], $sql_q);
		$ar_q = explode("\r\n", $sql_q);
		unset($ar_q[sizeof($ar_q)-1]);
		...
		$is_continue = ($is_continue && $this->post_queries($ar_q));
		/* Update for wordmap */
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'wordmap');
		$is_field_exists['date_created'] = 0;
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!$is_field_exists['date_created'])
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'wordmap` ADD `date_created` INT(10) UNSIGNED NOT NULL default \'0\' AFTER `term_match`');
		}
		...
		for (reset($arSql); list($kQ, $arV) = each($arSql);)
		{
			...
			$q1['dict_settings']['is_show_term_suggest'] = '1';
			$q1['dict_settings']['is_show_term_report'] = '1';
			$q1['dict_settings']['is_show_page_refresh'] = '0';
			$q1['dict_settings']['is_show_page_send'] = '1';
			$q1['dict_settings']['is_show_add_to_favorites'] = '0';
			$q1['dict_settings']['is_show_add_to_search'] = '1';
			$q1['dict_settings']['is_show_printversion'] = '1';
			$q1['dict_settings']['recent_terms_sorting'] = '1';
			$q1['dict_settings']['recent_terms_number'] = '10';
			$q1['dict_settings']['page_limit'] = '20';
			$q1['dict_settings']['page_limit_search'] = '20';
	...
	/* Update dictionaries */
	function upgrade_to_1_8_4_step_4()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.3', '1.8.4') );
		...
		if ($cntDicts > 0)
		{
			...
			$sql_file = $this->sys['path_install'].'/sql/glossword1_up184_dict_'.$this->sys['db_type'].'.sql';
	...
	/* */
	function upgrade_to_1_8_4_step_5()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.3', '1.8.4') );
