<?php
class gw_setup_upgrade_to_1_8_7 extends gw_setup
{
	/* */
	function upgrade_to_1_8_7_step_1()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.6', '1.8.7') );
		$this->oTpl->a( 'v:html_descr', $this->oL->m('1169') );

		$this->str_before .= '<div class="xu"><p><strong>';
		$this->str_before .= $this->oL->m('1244');
		$this->str_before .= '</strong></p><ol>';
		$this->str_before .= '<li class="red"><p>'.$this->oL->m('1250').'</p></li>';
		$this->str_before .= '<li><p>'.sprintf($this->oL->m('1251'), '<b>1.8.6</b>').'</p></li>';
		$this->str_before .= '</ol>';
	...
	/* */
	function upgrade_to_1_8_7_step_2()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.6', '1.8.7') );
	...
	function upgrade_to_1_8_7_step_3()
	{
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up187_structure_'.$this->gv['arpost']['db_type'].'.sql';
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up187_data.sql';
		$sql_q = implode('', file($sql_file));
		$sql_q = str_replace('{PREFIX}', $this->gv['arpost']['dbprefix'], $sql_q);
		$ar_q = explode("\r\n", $sql_q);
		unset($ar_q[sizeof($ar_q)-1]);
		...
		$ar_q = $q1 = array();
		/* Update a custom alphabetic order */
		$is_field_exists = array();
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'custom_az`');
		$is_field_exists['az_value_lc'] = 0;
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!isset($is_field_exists['az_value_lc']))
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'custom_az` ADD `az_value_lc` varbinary(16) NOT NULL DEFAULT \'\' AFTER `az_value`');
		}
		if (!isset($is_field_exists['az_int']))
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'custom_az` ADD `az_int` int(9) NOT NULL DEFAULT \'0\' AFTER `az_value_lc`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'custom_az` DROP INDEX `az_value`, ADD INDEX `az_value` (`id_profile` , `int_sort`, `az_int` )');
		}
		/* Update the history of changes */
		$is_field_exists = array();
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'history_terms`');
		$is_field_exists['term_a'] = 0;
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!$is_field_exists['term_a'])
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_a` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `crc32u`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_b` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_a`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_c` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_b`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_d` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_c`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_e` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_d`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_f` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_e`');
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` ADD `term_order` varchar(255) NOT NULL DEFAULT \'\' AFTER `term_uri`');
		}
		$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` CHANGE `term_1` varbinary(16) NOT NULL DEFAULT \'\'');
		$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` CHANGE `term_2` varbinary(16) NOT NULL DEFAULT \'\'');
		$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'history_terms` CHANGE `term_3` varbinary(16) NOT NULL DEFAULT \'\'');

		/* Update the list of dictionaries */
		$is_field_exists = array();
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'dict`');
		$is_field_exists['term_a'] = 0;
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!isset($is_field_exists['id_vkbd']))
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'dict` ADD `id_vkbd` SMALLINT(5) UNSIGNED NOT NULL DEFAULT \'0\' AFTER `id_custom_az`');
		}
		/* */
		...
		/* Topics */
		$is_field_exists = array();
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'topics');
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!isset($is_field_exists['id_user']))
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'topics` ADD `id_user` SMALLINT( 5 ) UNSIGNED NOT NULL DEFAULT \'2\' AFTER `id_topic`');
		}


		/* Custom pages */
		$is_field_exists = array();
		$arD = $this->oDb->sqlExec('DESCRIBE `'. $this->sys['tbl_prefix'].'pages');
		foreach($arD as $arV)
		{
			 $is_field_exists[$arV['Field']] = 1;
		}
		if (!isset($is_field_exists['id_user']))
		{
			$this->oDb->sqlExec('ALTER TABLE `'.$this->sys['tbl_prefix'].'pages` ADD `id_user` SMALLINT( 5 ) UNSIGNED NOT NULL DEFAULT \'2\' AFTER `id_page`');
		}
		$this->oDb->sqlExec('UPDATE `'.$this->sys['tbl_prefix'].'pages` SET page_php_1 = "gwtk\_header(\$sys[\'server\_dir\'].\'/\'.\$sys[\'file\_admin\']);" WHERE id_page = "4"');
		/* Custom pages data */
		...
		$this->post_queries( array('TRUNCATE TABLE `'.$this->sys['tbl_prefix'].'custom_az_profiles`') );
	...
	/* Update dictionaries */
	function upgrade_to_1_8_7_step_4()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.6', '1.8.7') );
		...
		$this->ar_status[] = $this->oL->m('1234').': <strong>' . sizeof($arSql) . '</strong>';
		...
		for (reset($arSql); list($arK, $arV) = each($arSql);)
		{
			...
			$q1['dict_settings']['is_audio'] = '0';
			$q1['dict_settings'] = serialize($q1['dict_settings']);
			$sql = gw_sql_update($q1, $this->sys['tbl_prefix'].'dict', 'id = '. $arV['id']);
		...
		if ($cntDicts > 0)
		{
			...
			/* Advanced update for dictionary structure */
			$is_field_exists = array();
			$arT = $this->oDb->sqlExec('DESCRIBE `'. $arDicts[0].'`');
			foreach($arT as $arV)
			{
				$is_field_exists[$arV['Field']] = 1;
			}
			if (!isset($is_field_exists['term_a']))
			{
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_a` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `crc32u`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_b` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_a`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_c` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_b`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_d` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_c`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_e` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_d`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_f` int(9) unsigned NOT NULL DEFAULT \'0\' AFTER `term_e`');
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `term_order` varchar(255) NOT NULL DEFAULT \'\' AFTER `term_uri`');
				$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET `term_order` = UPPER(term)');
			}
			if (!isset($is_field_exists['id_user']))
			{
				$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` ADD `id_user` SMALLINT( 5 ) UNSIGNED NOT NULL DEFAULT \'2\' AFTER `id`');
			}
			$this->oDb->sqlExec('ALTER TABLE `'.$arDicts[0].'` DROP INDEX `term123`, ADD INDEX `term123` ( `is_active` , `term_1` , `term_2` , `term_3` , `date_created` )');
			$this->oDb->sqlExec('UPDATE `'.$arDicts[0].'` SET date_created = (date_created - 86400), term_uri = LOWER(term), is_active = "1"');
		}
		$arDicts = array();
		...
		$cntDicts = sizeof($arDicts);
		$this->ar_status[] = $this->oL->m('1234').': <strong>' . $cntDicts . '</strong>';
	...
	/* */
	function upgrade_to_1_8_7_step_5()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.6', '1.8.7') );
...
#$this->sys['is_debug'] = 1;
		...
		/* COMPLETE */
		$this->ar_status[] = '<strong class="green">'.$this->oL->m('1249').'</strong>';
		...
		switch ($step)
		{
			case 2:
				...
				$tmp .= $this->get_form_tr($this->oL->m('1207'), $this->oForm->field('input', 'arpost[server_host]', $this->gv['arpost']['server_host']) .
							'<div>ex. glossword.info</div>'
						);
				$tmp .= $this->get_form_tr($this->oL->m('1210'),
						$this->oForm->field('input', 'arpost[server_dir]', $this->gv['arpost']['server_dir']) .
							'<div>ex. /dict</div>' .
							$this->oL->m('1211')
						);
				$tmp .= '</tbody></table>';
