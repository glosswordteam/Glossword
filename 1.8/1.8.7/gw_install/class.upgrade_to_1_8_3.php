<?php
class gw_setup_upgrade_to_1_8_3 extends gw_setup
{
	/* */
	function upgrade_to_1_8_3_step_1()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.2', '1.8.3') );
		...
		$this->str_before .= '<li><p>'.sprintf($this->oL->m('1251'), '1.8.2').'</p></li>';
		$this->str_before .= '</ol>';
		...
		if ($is_continue)
		{
			...
			$is_continue = 0;
			if ( file_exists($this->sys['path_temporary']) && is_writeable($this->sys['path_temporary'].'/t') )
			{
				$this->ar_status[2] .= get_html_item_progress(sprintf($this->oL->m('1173'), $this->sys['path_temporary'].'/t/'), 1);
				$is_continue = 1;
			}
			else
			{
				$this->ar_status[3] .= get_html_item_progress(sprintf($this->oL->m('1174'), $this->sys['path_temporary'].'/t/'), 3);
			}
		}
		/* Get PHP extensions */
		if ($is_continue)
	...
	/* */
	function upgrade_to_1_8_3_step_2()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.2', '1.8.3') );
...
#			$this->gv['arpost']['pass_new'] = $this->oFunc->text_make_uid(8, 0);
	...
	function upgrade_to_1_8_3_step_3()
	{
		...
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up183_structure_'.$this->gv['arpost']['db_type'].'.sql';
		...
		/* Insert or change database data */
		$sql_file = $this->sys['path_install'].'/sql/glossword1_up183_data.sql';
		$sql_q = implode('', file($sql_file));
		$sql_q = str_replace('{PREFIX}', $this->gv['arpost']['dbprefix'], $sql_q);
		$ar_q = explode("\r\n", $sql_q);
		unset($ar_q[sizeof($ar_q)-1]);
...
#		$arSql = $this->oDb->sqlExec('SELECT * FROM `'.$this->sys['tbl_prefix'].'dict`');
	...
	/* Update dictionaries */
	function upgrade_to_1_8_3_step_4()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.2', '1.8.3') );
		...
		if ($cntDicts > 0)
		{
			...
			$sql_file = $this->sys['path_install'].'/sql/glossword1_up183_dict_'.$this->sys['db_type'].'.sql';
			...
			/* Updating terms */
			$sql_t1 = sprintf("SELECT id, term, term_1, term_2, term_3, defn FROM `%s`", $arDicts[0]);
			$arT = $this->oDb->sqlExec($sql_t1);
			for (reset($arT); list($kT, $vT) = each($arT);)
			{
				$sql_t2 = sprintf("UPDATE `%s` SET `crc32u` = '%u' WHERE id = '%d'",
									$arDicts[0], crc32($vT['id'].$vT['term'].$vT['term_1'].$vT['term_2'].$vT['term_3'].$vT['defn']), $vT['id']
							);
				if ($this->sys['is_debug'])
				{
					#$this->ar_status[] = $sql_t2.';';
				}
				else
				{
					$this->oDb->sqlExec($sql_t2);
				}
			}
		}
		$arDicts = array();
		...
		if ($cntDicts > 0)
		{
			$this->ar_status[] = gw_next_step($is_continue, 'step=4&a='.$this->gv['a'].'&id_dict='.$this->gv['id_dict'].'&'.GW_LANG_I.'='.$this->gv[GW_LANG_I]);
		}
		else
		{
	...
	/* */
	function upgrade_to_1_8_3_step_5()
	{
		$this->ar_tpl[] = 'i_step_3.html';
		$this->oTpl->a( 'v:html_title', sprintf($this->oL->m('1165'), '1.8.2', '1.8.3') );
		$this->oTpl->a( 'v:html_descr', $this->oL->m('1246') );

		$ar_q[] = 'CHECK TABLE `'.$this->sys['tbl_prefix'].'stat_search';
		$ar_q[] = 'OPTIMIZE TABLE `'.$this->sys['tbl_prefix'].'stat_search';
		$ar_q[] = 'CHECK TABLE `'.$this->sys['tbl_prefix'].'theme_settings`';
		$ar_q[] = 'OPTIMIZE TABLE `'.$this->sys['tbl_prefix'].'theme_settings``';
