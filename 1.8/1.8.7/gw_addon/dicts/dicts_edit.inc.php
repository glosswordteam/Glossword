...
if (!defined('IN_GW'))
{
	die('<!-- $Id: dicts_edit.inc.php 404 2008-04-07 11:37:59Z yrtimd $ -->');
}
...
{
	$this->oDb->sqlExec('UPDATE `'.$this->sys['tbl_prefix'].'dict`
						SET `is_active` = "0"
						WHERE `id` = "'.$this->gw_this['vars']['id'].'"' );
	postQuery($arQ, GW_ACTION.'='.GW_A_BROWSE . '&'.GW_TARGET.'='.$this->gw_this['vars'][GW_TARGET].'&w1='.$this->gw_this['vars']['w1'], $this->sys['isDebugQ'], 0);
	return;
}
...
else
{
	...
	$arOnOff = array('is_trsp', 'is_trns', 'is_abbr', 'is_defn', 'is_usg', 'is_src', 'is_syn', 'is_audio',
						...
						'is_filter_stopwords', 'is_show_az', 'is_leech','dict_uri',
...
#$this->sys['isDebugQ'] = 1;
		...
		/* Dictionary URI */
		$q['dict_uri']      = (!$arPost['dict_uri']) ? $q['title'] : $arPost['dict_uri'];
		    ...
		    && !getTableInfo($arPost['tablename']))
		{
			...
			$queryA[] = $this->oSqlQ->getQ('rename-tbl', $arPost['tablename_old'], $arPost['tablename']);
		}
		...
		$queryA[] = gw_sql_update($q, TBL_DICT, 'id = "'.$this->gw_this['vars']['id'].'"');
		/* Clear cache */
...
#		gw_sys_dict_check(TBL_MAP_USER_TERM);
		...
		$this->str .= postQuery($queryA, $str_url.'&note_afterpost='.$this->oL->m('1332'), $this->sys['isDebugQ'], 0);
	}
}
