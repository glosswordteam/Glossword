...
if (!defined('IN_GW'))
{
	die('<!-- $Id: dicts_admin.php 403 2008-04-06 23:16:56Z yrtimd $ -->');
}
/* */
...
#		$this->oL->getCustom('addon_'.$this->component, $this->gw_this['vars'][GW_LANG_I].'-'.$this->gw_this['vars']['lang_enc'], 'join');
		...
		return '<div class="xt gray" style="margin-bottom:1em;border-bottom: 2px solid #EEE">'.
	...
	function get_form_dict($vars, $runtime = 0, $ar_broken = array(), $ar_req = array())
	{
		...
		else
		{
			...
			$str_form .= '<tr>'.
						'<td class="'.$v_class_1.'">' . $this->oL->m('topic')  . ':' . $ar_req_msg['id_topic'] . '</td>'.
						'<td class="'.$v_class_2.' gray">' . $ar_broken_msg['id_topic']
			...
			{
				$str_form .= ' ['.$this->oHtml->a($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_BROWSE.'&'.GW_TARGET.'='.GW_T_TOPICS, $this->oL->m('3_edit')).']';
			}
			$str_form .= '</td>'.
						'</tr>';
			$str_form .= '</tbody></table>';
...
#			$oForm->setTag('select', 'class',  'input75');
			...
			$str_form .= '<tr>'.
						'<td class="'.$v_class_1.'">' . $this->oL->m('visual_theme') . ':' . $ar_req_msg['visualtheme'] . '</td>'.
						'<td class="'.$v_class_2.' gray">' . $oForm->field('select', 'arPost[visualtheme]', $vars['visualtheme'], 0, array_merge(array($this->oL->m('1270')), $this->gw_this['ar_themes_select']));
			if ($this->oSess->is('is-sys-settings'))
			{
				$str_form .= ' ['.$this->oHtml->a($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_BROWSE.'&'.GW_TARGET.'=visual_theme', $this->oL->m('3_edit')).']';
			}
			$str_form .= '</td>'.
						'</tr>';
			...
			$str_form .= '<tr>'.
						'<td class="'.$v_class_1.'">' . $this->oL->m('custom_az') . ':' . $ar_req_msg['id_custom_az'] . '</td>'.
						'<td class="'.$v_class_2.' gray">' . $oForm->field('select', 'arPost[id_custom_az]', $vars['id_custom_az'], 0, $ar_custom_az);
			if ($this->oSess->is('is-sys-settings'))
			{
				$str_form .= ' ['.$this->oHtml->a($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_BROWSE.'&'.GW_TARGET.'=custom-az&tid='.$vars['id_custom_az'], $this->oL->m('3_edit')).']';
			}
			$str_form .= '</td>'.
						'</tr>';
			...
			$str_form .= '<tr>'.
						'<td class="'.$v_class_1.'">' . $this->oL->m('virtual_keyboard') . ':' . $ar_req_msg['id_vkbd'] . '</td>'.
						'<td class="'.$v_class_2.' gray">' . $oForm->field('select', 'arPost[id_vkbd]', $vars['id_vkbd'], 0, $ar_vkbd);
			if ($this->oSess->is('is-sys-settings'))
			{
				$str_form .= ' ['.$this->oHtml->a($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_BROWSE.'&'.GW_TARGET.'=virtual-keyboards&tid='.$vars['id_vkbd'], $this->oL->m('3_edit')).']';
			}
			$str_form .= '</td>'.
						'</tr>';
			...
			$oForm->setTag('input', 'dir', $this->sys['css_align_numbers'] );
#			$oForm->setTag('input', 'onkeyup', 'gwJS.strNormalize(this)');
			$str_form .= '<tr>'.
