...
#		$sql_group_by = ' GROUP BY sr.date_created';
	...
	function _print_report($arSql)
	{
		...
		$strR .= '<td>'. $this->get_menu_period().'</td>';
		...
		$strR .= '<td>' . $this->oL->m('srch_matches') .': <b>'. $this->oFunc->number_format($this->int_found, 0, $this->oL->languagelist('4')) . '</b></td>';
		$strR .= '<td style="width:50%;text-align:'.$this->sys['css_align_right'].'">'. $str_pages.'</td>';
		$strR .= '</tr></tbody></table>';
