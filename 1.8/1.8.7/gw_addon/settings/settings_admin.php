...
class gw_addon_settings_admin extends gw_addon
{
	...
	function get_form($vars, $runtime = 0, $arBroken = array(), $arReq = array())
	{
		...
		$oForm->Set('method', 'get');
		$oForm->arLtr = array('arPost[y_email]',
						 'arPost[locale_name]', 'arPost[visualtheme]','arPost[path_log]'
		);
		## ----------------------------------------------------
		##
		...
		$str_form .= '<tr>'.
					...
					'<td class="'.$v_class_2.'">' . $this->oL->m('1132') . $arReqMsg['avatar_max_kb'] . '</td>'.
					'</tr>';
		$str_form .= '</tbody></table>';
		$str_form .= '</fieldset>';
...
#	$oForm->setTag('input', 'dir', '');
	...
	function get_form_support($vars)
	{
		...
		$str_form .= '<tr><td style="width:20%" class="'.$trClass.'">'.$this->oL->m('y_name').'</td>';
		$str_form .= '<td>'.
					$oForm->field('input', 'name', $this->oSess->user_get("user_name")).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('y_email').'</td>';
		$str_form .= '<td>'.
					$oForm->field('input', 'email', $this->oSess->user_get("user_email")).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('1025').'</td>';
		$str_form .= '<td class="disabled">'.
					$this->sys['version'].
					$oForm->field('hidden', 'arPost[gw_version]', $this->sys['version']).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('1026').'</td>';
		$str_form .= '<td class="disabled">'.
					PHP_VERSION.
					$oForm->field('hidden', 'arPost[php_version]', PHP_VERSION).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('1027').'</td>';
		$str_form .= '<td class="disabled">'.
					PHP_OS.
					$oForm->field('hidden', 'arPost[php_os]', PHP_OS).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('1028').'</td>';
		$str_form .= '<td class="disabled">'.
					(isset($arSql[0]['v']) ? $arSql[0]['v'] : '').
					$oForm->field('hidden', 'arPost[db_version]', (isset($arSql[0]['v']) ? $arSql[0]['v'] : '')).
					'</td></tr>';
		$str_form .= '<tr><td class="'.$trClass.'">'.$this->oL->m('topic').'</td>';
		$str_form .= '<td>'.
					htmlFormsSelect($this->ar_msg_topics, $vars['id_topic'], 'id_topic', '', 'width:20em', $this->oL->languagelist("1")).
					'</td></tr>';
		$str_form .= '<tr style="vertical-align:top"><td class="'.$trClass.'">'.$this->oL->m('message').'</td>';
		$str_form .= '<td>';
		$str_form .= '<textarea name="arPost[message]" style="height:15em;border:1px solid #BCC8E2;overflow:auto;width:100%">'.
		...
		$str_form .= '<col width="1%"/><col width="99%"/><tbody>';
		$str_form .= '<tr>'.
					'<td class="' . $trClass . '">' .
					$oForm->field('checkbox', "arPost[is_attach]", $vars['is_attach']) .
					'</td><td class="' . $trClass . '">'.
