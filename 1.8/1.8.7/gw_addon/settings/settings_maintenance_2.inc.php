...
{
	global $is, $arStatus, $oL, $sys, $tid, $gw_this;
	$str = '';
	...
	if ($intKeyUnused > 0)
	{
		...
		{
			$arStatus[] = array('&#160;');
			$arStatus[] = array(sprintf('<strong>%s</strong>', $oL->m('1016')), sprintf('<a href="%s">%s</a>',
			append_url($sys['page_admin'].'?'.GW_ACTION.'='.$gw_this['vars'][GW_ACTION].'&tid='.$tid.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&is=1'),
			$oL->m('3_yes')));
			$arStatus[] = array('&#160;');
		}
	}
