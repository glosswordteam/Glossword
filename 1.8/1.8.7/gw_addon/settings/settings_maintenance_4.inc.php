...
function gw_get_sql_cache()
{
	...
	{
		$arStatus[] = array(sprintf('<strong>%s</strong>', $oL->m('1023')), sprintf('<a href="%s">%s</a>',
		append_url($sys['page_admin'].'?'.GW_ACTION.'='.$gw_this['vars'][GW_ACTION].'&tid='.$gw_this['vars']['tid'].'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&isConfirm=2'),
		$oL->m('3_clean')));
		$arStatus[] = array('&#160;');
	}
	return $arStatus;
...
function gw_get_file_cache()
{
	...
	{
		$arStatus[] = array(sprintf('<strong>%s</strong>', $oL->m('1023')), sprintf('<a href="%s">%s</a>',
		append_url($sys['page_admin'].'?'.GW_ACTION.'='.$gw_this['vars'][GW_ACTION].'&tid='.$gw_this['vars']['tid'].'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&isConfirm=1'),
		$oL->m('3_clean')));
		$arStatus[] = array('&#160;');
	}
	return $arStatus;
