	...
	function alpha()
	{
		...
		foreach ($fields as $f)
		{
			$vars[$f] = (isset($this->gw_this['vars']['arPost'][$f])
				&& (trim($this->gw_this['vars']['arPost'][$f]) != '' ))
			? $this->gw_this['vars']['arPost'][$f]
			: '';
		}
		/* */
		switch ($this->gw_this['vars']['uid'])
		{
			...
			case 'sendpage':
				...
				$vars['message'] = ($vars['message']) ? $vars['message']
					: sprintf($this->oL->m('1276'), CRLF.$vars['title'], CRLF.str_replace('&amp;', '&', $vars['url']));
			break;
			default:
