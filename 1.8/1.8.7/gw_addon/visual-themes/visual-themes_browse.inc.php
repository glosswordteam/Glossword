...
if ( ($this->gw_this['vars']['p'] < 1) || ($this->gw_this['vars']['p'] > $int_pages) ){ $this->gw_this['vars']['p'] = 1; }
...
$this->str .= '<th>' . $this->oL->m('1140') . '</th>';
$this->str .= '<th style="width:35%">' . $this->oL->m('action') . '</th>';
...
while (list($k, $arV) = each($arSql))
{
	...
	$img_src = file_exists($this->sys['path_temporary'].'/t'.'/'.$arV['id_theme'].'/thumbnail.png')
				? $this->sys['path_temporary'].'/t'.'/'.$arV['id_theme'].'/thumbnail.png'
				: 'img/0x0.gif';
	...
	$this->str .= '</td>';
	$this->str .= '<td style="text-align:center;vertical-align:middle" class="xt">[';
	$this->str .= $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$this->component.'&tid=' . $arV['id_theme'], $this->oL->m('3_edit') );
	$this->str .= '] [';
	$this->str .= $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_IMPORT.'&'.GW_TARGET.'='.$this->component.'&tid=' . $arV['id_theme'], $this->oL->m('3_import') );
	$this->str .= '] [';
	$this->str .= $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_EXPORT.'&'.GW_TARGET.'='.$this->component.'&tid=' . $arV['id_theme'], $this->oL->m('3_export' ) );
	if ($arV['id_theme'] != 'gw_admin')
	{
		$this->str .= '] [';
		$this->str .= $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_EDIT.'&remove=1&'.GW_TARGET.'='.$this->component.'&tid=' . $arV['id_theme'], $this->oL->m('3_remove') );
	}
	$this->str .= ']</td>';
	/* 1.8.7: Turn on/off */
