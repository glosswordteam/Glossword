	...
	function get_tpl_pages($id_theme)
	{
		$ar_theme_gp = array();
			...
			else
			{
				...
				$ar_theme_gp[7] = $this->oHtml->a( $url_part.'&w1=7', $this->oL->m('term') ) . ' (' . $this->oHtml->a( $url_part.'&w1=8', $this->oL->m('printversion')).')';
...
#			$arV['settings_value'] = str_replace(array('{', '}'), array('{%', '%}'), $arV['settings_value']);
			...
			else
			{
				...
				$str_tpl_ctrl = '<div style="border-top:1px solid #CCC; float:right">'. $this->oHtml->a($url_tpl_add, $this->oL->m('3_add')) . ' | ';
				...
				$str_form .= '<tr>'.
					'<td class="'.$v_class_2.'">'.
					'<div style="height:2em">' .  $str_tpl_ctrl . ' <b>'. $arV['settings_key'] . '</b>: ' . '</div>'.
	...
	function get_form_export($vars)
	{
		...
		$str_form .= '<tr>'.
			...
			for (; list($k, $page) = each($vars['tpl_pages']);)
			{
				...
				$str_form .= '<tr><td style="width:1%">' . $str_checkbox.'</td><td>'. $str_external_link . $str_label.'</td></tr>';
			}
			$str_form .= '</tbody></table>';
		$str_form .= '</td></tr>';
