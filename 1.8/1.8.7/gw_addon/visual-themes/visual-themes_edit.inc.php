...
if ($this->gw_this['vars']['post'] == '')
{
	...
	$this->str .= '<div class="xt" style="padding-bottom:5px">';
	$this->str .= implode(' - ', $ar_tpl_pages).'</div>';
	$this->cur_template = isset($ar_tpl_pages[$this->gw_this['vars']['w1']]) && ($this->gw_this['vars']['w1'] != 'css')
							? strip_tags($ar_tpl_pages[$this->gw_this['vars']['w1']])
							: $this->oL->m('1137');
...
#				if (preg_match("/^is_/", $arV['settings_key']))
	...
	else
	{
		...
		$q1 = array();
		if (!isset($vars['is_active'])){ $vars['is_active'] = 0; }
