...
if (!defined('IN_GW'))
{
	die('<!-- $Id: users_admin.php 409 2008-04-08 15:27:39Z yrtimd $ -->');
}
/* */
class gw_addon_users_admin extends gw_addon
{
	...
	function gw_addon_users_admin()
	{
		...
		{
			$this->gw_this['vars']['tid'] = $this->oSess->id_user;
		}
		/* Check permissions on editing other user`s profile */
		if (($this->gw_this['vars']['tid'] != $this->oSess->id_user) && !$this->oSess->is('is-users') )
		{
			$this->gw_this['vars']['tid'] = $this->oSess->id_user;
		}
		/* Set special attribute for editing further options */
		$this->sys['is_profile_owner'] = 0;
		if ($this->gw_this['vars']['tid'] == $this->oSess->id_user)
		{
...
#prn_r( $vars );
		...
		/* */
		if ($this->ar_state['is_user_edit'] && $this->gw_this['vars']['tid'] != 1)
		{
		...
		if ($this->oSess->is('is-users') || $this->oSess->is('is-password'))
		{
			/* allow edit username */
			$str_form .= '<tr>'.
		...
		if ($this->ar_state['is_user_edit'])
		{
			...
			{
			$str_form .= ' [ <a href="'.
						append_url($this->sys['page_admin']. '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.GW_T_USERS.'&remove=1&tid='.$this->gw_this['vars']['tid']).
						'">'. $this->oL->m('3_remove') .'</a> ]';
			}
			$str_form .= '</td>';
			$str_form .= '</tr>';
		}
...
#					'</tr>';
		...
		if ($this->gw_this['vars'][GW_ACTION] == GW_A_EDIT)
		{
			$str_form .= '<tr>'.
						...
						'<td class="'.$v_class_2.'"><div style="margin-bottom:4px">' . $this->oL->m('1103').': <strong>' .
							date_extract_int( $this->oSess->user_get_time(), "%H:%i:%s %d %M %Y") .
							'</strong></div>'.
		...
		{
			$vars['avatar_img_html'] = '<img width="'.$vars['user_settings']['avatar_img_x'].'" height="'.$vars['user_settings']['avatar_img_y'].'" src="'.$this->sys['path_temporary'].'/a/'.$vars['user_settings']['avatar_img'].'" />';
		}
		...
		$str_form .= '<tr>'.
					...
					'<td class="'.$v_class_2.'">' . $vars['avatar_img_html'] . '</td>'.
					'</tr>';
		...
		for (reset($this->gw_this['ar_dict_list']); list($k, $arDictParam) = each($this->gw_this['ar_dict_list']);)
		{
			...
			$str_external_link = $arDictParam['is_active']
						? '<a href="'.$this->sys['page_index'].'?a=index&amp;d='. $arDictParam['id'] .'" onclick="window.open(this.href);return false;">&gt;&gt;&gt;</a> '
			...
			$str_form .= '<tr>'.
					'<td class="'.$v_class_1.'">' . $str_checkbox.'</td><td class="'.$v_class_2.'">'.$str_external_link .$str_label.'</td>'.
					'</tr>';
			$cnt++;
