...
if (!defined('IN_GW'))
{
	die('<!-- $Id: users_edit.inc.php 406 2008-04-07 13:17:07Z yrtimd $ -->');
}
...
{
	$arSql = $this->oSess->user_load_values($this->gw_this['vars']['tid']);
	/* No such user */
	if (empty($arSql))
	{
...
else
{
	...
	$id_user = ($this->gw_this['vars']['tid'] != '') ? $this->gw_this['vars']['tid'] : $this->oSess->id_user;
...
#prn_r( $ar_user, 'user_load_values' );
	...
	if (!empty($arPost['pass_new']) || !empty($arPost['pass_confirm']))
	{
		...
		else if ($this->gw_this['vars']['tid'] == $this->oSess->id_user)
		{
...
#		$arSql = $this->oSess->user_load_values($this->gw_this['vars']['tid']);
		...
		$this->str .= $this->get_form_user($arSql, 0, 0, $ar_req_fields);
	}
...
#$this->sys['isDebugQ'] = 1;
		...
		if ($this->sys['is_profile_owner'])
		{
			...
			setcookie('gw_'.GW_LANG_I.$this->sys['token'], $this->gw_this['vars'][GW_LANG_I], $this->sys['time_now']+$this->sys['time_sec_y'], '/', $this->sys['server_host'] );
			setcookie('gw_is_save_'.GW_LANG_I.$this->sys['token'], 1,  $this->sys['time_now']+$this->sys['time_sec_y'], '/', $this->sys['server_host'] );
		}
