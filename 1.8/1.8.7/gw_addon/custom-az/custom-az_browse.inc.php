...
$this->str .= '<ul class="noindent xu"><li>';
...
if ($this->gw_this['vars']['tid'])
{
	...
	while (list($k, $arV) = each($arSql))
	{
		...
		$this->str .= '<tr id="az-'.$arV['id_letter'].'" style="color:'.$this->ar_theme['color_5'].';background:'.$bgcolor.'">';
		$this->str .= '<td class="xt gray" style="text-align:'.$this->sys['css_align_right'].'">' .  $cnt_row . '</td>';
		...
		$this->str .= '<td class="xt" style="text-align:'.$this->sys['css_align_left'].'">[';
		$this->str .= ($isUp) ? $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$this->component.'&mode=up&tid='.$this->gw_this['vars']['tid'].'&w1='.$arV['id_letter'], $this->oL->m('3_up')) : $this->oL->m('3_up');
		$this->str .= '] [';
		$this->str .= ($isDn) ? $this->oHtml->a( $this->sys['page_admin'] . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$this->component.'&mode=down&tid='.$this->gw_this['vars']['tid'].'&w1='.$arV['id_letter'], $this->oL->m('3_down')) : $this->oL->m('3_down');
		$this->str .= '] [';
