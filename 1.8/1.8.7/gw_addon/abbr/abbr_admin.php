...
class gw_addon_abbr_admin extends gw_addon
{
	...
	function browse()
	{
		...
		while (list($k, $arV) = each($arSql))
		{
			...
			$strR .= '<tr id="abbr'.$arV['id_abbr'].'" style="color:'.$this->ar_theme['color_5'].';background:'.$bgcolor.'">';
			$strR .= '<td class="xt gray" style="text-align:'.$this->sys['css_align_right'].'">' .  $cnt_row . '</td>';
			$strR .= '<td class="xt gray">'.sprintf("%03d", $arV['id_abbr']).'</td>';
			$strR .= '<td class="xu"><table><tr><td>';
			$strR .= $this->oHtml->a(
			...
			$strR .= '</td>';
			$strR .= '<td class="xt" style="text-align:center">[';
			...
			$strR.= '<td class="xt gray" style="width:1%;text-align:center">';
			$strR .= '['.($arV['is_active']
