...
else
{
	...
	if (!$isPostError) /* single check is ok */
	{
		...
		$fileS = getExportFilename(
				$this->sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'],
				$cntFiles,
				FORMAT_EXT
			 );
		...
		{
			$strQ = '';
			$filename = $sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'] . '_dict.' . FORMAT_EXT;
			$this->str .= '<li><span class="gray">';
			$this->str .= $oHtml->a($filename, $filename) . '</span>&#8230; ';
			$strQ .= '# <!-- ' . $filename . ' -->' . CRLF;
			...
			$this->str .= ( $isWrite ?  'ok (' . $this->oFunc->number_format(strlen($strQ), 0, $oL->languagelist('4')) . " " . $oL->m('bytes') . ')' : $oL->m('error') ) . '</li>';
		}
		/* 15 july 2003: Export dictionary statistics */
		if ($vars['is_dictstats'])
		{
			$strQ = '';
			$filename = $sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'] . '_stats.' . FORMAT_EXT;
			$this->str .= '<li><span class="gray">';
			$this->str .= $oHtml->a($filename, $filename) . '</span>&#8230; ';
			$strQ .= '# <!-- ' . $filename . ' -->' . CRLF;
		...
		{
			$strQ = '';
			$filename = $sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'] . '_structure.' . FORMAT_EXT;
			$this->str .= '<li><span class="gray">';
			$this->str .= $oHtml->a($filename, $filename) . '</span>&#8230; ';
			$strQ .= '# <!-- ' . $filename . ' -->' . CRLF;
...
#					}
				...
				$filename = sprintf(getExportFilename(
						$sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'].'_map_user_to_term',
						$cntFiles,
						FORMAT_EXT
					 ), ($i + 1), $cntFiles
				);
		...
		if ($vars['is_keywords'])
		{
			...
			for ($i = 0; $i < $cntFiles; $i++)
			{
				...
				$fileS = getExportFilename(
					$sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'] . '_'.TBL_WORDMAP,
					$cntFiles,
					FORMAT_EXT
				 );
				$filename = sprintf($fileS, ($i + 1), $cntFiles) ;
				...
				// File name for wordmap
				$fileS = getExportFilename(
					$sys['path_export'] . '/' . sprintf("%05s", $this->gw_this['vars']['id']) . '_' . $arDictParam['tablename'] . '_'.TBL_WORDLIST,
					$cntFiles,
					FORMAT_EXT
				 );
				$filename = sprintf($fileS, ($i + 1), $cntFiles);
