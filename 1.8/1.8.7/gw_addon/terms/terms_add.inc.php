...
if (!defined('IN_GW'))
{
	die('<!-- $Id: terms_add.inc.php 408 2008-04-08 08:32:12Z yrtimd $ -->');
}
...
if (!$this->gw_this['vars']['id'])
{
	...
	for (reset($this->gw_this['ar_dict_list']); list($k, $v) = each($this->gw_this['ar_dict_list']);)
	{
		...
		{
			$this->str .= '<li>';
			$this->str .= $this->oHtml->a($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_ADD .'&'. GW_TARGET.'='.GW_T_TERMS. '&id='.$v['id'], 'ID:'.$v['id'].' '.$v['title']. ' ('.$v['int_terms'].')' );
			$this->str .= '</li>';
			$cnt_dict++;
		}
	}
...
#$this->sys['isDebugQ'] = 1;
	...
	if (!$isPostError) // final update
	{
		...
		$arPre['is_parse_url'] = isset($arPost['is_parse_url']) ? 1 : 0;
		/* Set term parameters, $arTermParam['id'] is inside gwAddTerm()  */
...
#prn_r( $arPre );
		...
		{
			/* Redirect to... */
			$str_url = gw_after_redirect_url( $arPost['after'], $arTermParam['id'] );
