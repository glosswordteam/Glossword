...
if (!defined('IN_GW'))
{
	die('<!-- $Id: terms_edit.inc.php 404 2008-04-07 11:37:59Z yrtimd $ -->');
}
...
{
	/* Publish term */
	$sql = 'UPDATE `%s` SET `is_active` = "1" WHERE `id` IN (%s)';
	...
	for (reset($this->gw_this['vars']['arPost']['ar_id']); list($k1, $id_term) = each($this->gw_this['vars']['arPost']['ar_id']);)
	{
		...
		if (!empty($arCurrent))
		{
			...
			$ar_query[] = 'UPDATE `'.$this->sys['tbl_prefix'].'history_terms` SET `is_active` = "1", `id_user` = "'.$this->oSess->user_get('id_user').'" WHERE `id` = "' . $arCurrent['id'] . '"';
		}
	}
}
...
{
	/* Unpublish term */
	$sql = 'UPDATE `%s` SET `is_active` = "0" WHERE `id` IN (%s)';
	$sql = sprintf($sql, $arDictParam['tablename'], implode(',', $this->gw_this['vars']['arPost']['ar_id']));
	$ar_query[] = $sql;
	for (reset($this->gw_this['vars']['arPost']['ar_id']); list($k1, $id_term) = each($this->gw_this['vars']['arPost']['ar_id']);)
	{
		...
		if (!empty($arCurrent))
		{
			$arCurrent = $arCurrent[0];
			/* Restore removed term, change User ID */
			$ar_query[] = 'UPDATE `'.$this->sys['tbl_prefix'].'history_terms` SET `is_active` = "0", `id_user` = "'.$this->oSess->user_get('id_user').'" WHERE `id` = "' . $arCurrent['id'] . '"';
		}
	}
}
...
/* Check permission to edit the term */
/**
 * Conditions: 
...
/* The histroy of changes, rollback, keywords */
if ($this->gw_this['vars']['w1'] == 'viewhistory')
{
	$ar_term_menu = array();
	if ($arTermParam['is_active'] != 3)
	{
		$ar_term_menu[] = '<a href="'.$this->oHtml->url_normalize($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('3_edit').'</a>';
		$ar_term_menu[] = '<a href="'.$this->oHtml->url_normalize($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&w1=viewkeywords&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('1284').'</a>';
		$ar_term_menu[] = '<a class="ext" href="'.$this->oHtml->url_normalize($this->sys['page_index'].'?'.GW_ACTION.'='.GW_T_TERM.'&d='.$arDictParam['uri'].'&'.GW_TARGET.'='.$arTermParam['uri']).'" onclick="window.open(this.href);return false">'.$this->oL->m('1283').'</a>';
	}
	$this->str .= getFormTitleNav( $this->oL->m('term') . ' <div class="gray">'.implode(' | ', $ar_term_menu).'</div>' );
	...
	$arCurrent = $this->oDb->sqlExec($this->oSqlQ->getQ('get-history-by-date',$arTermParam['date_modified']));
	...
	for (; list($arK, $arV) = each($arSql);)
	{
		...
		$this->str .= '<td class="xt">' . $cnt . '</td>';

			$str_edit = $this->oL->m('1295');
			...
			$this->str .= '<td class="xt">[&#160;' . $str_edit . '&#160;]</td>';
...
#$sys['isDebugQ'] = 1;
	...
	$this->str .= postQuery($arQ, $str_url, $sys['isDebugQ'], 0);
}
elseif ($this->gw_this['vars']['w1'] == 'viewkeywords')
{
	...
	if ($arTermParam['is_active'] != 3)
	{
		$ar_term_menu[] = '<a href="'.$this->oHtml->url_normalize( $this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&w1=viewhistory&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('1294').'</a>';
		$ar_term_menu[] = '<a href="'.$this->oHtml->url_normalize( $this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('3_edit').'</a>';
		$ar_term_menu[] = '<a class="ext" href="'.$this->oHtml->url_normalize( $this->sys['page_index'].'?'.GW_ACTION.'='.GW_T_TERM.'&d='.$arDictParam['uri'].'&'.GW_TARGET.'='.$arTermParam['uri']).'" onclick="window.open(this.href);return false">'.$this->oL->m('1283').'</a>';
	}
	$this->str .= getFormTitleNav( $this->oL->m('term') . ' <div class="gray">'.implode(' | ', $ar_term_menu).'</div>' );
...
if ($this->gw_this['vars']['post'] == '')
{
	...
	if (!isset($this->gw_this['vars']['arPost']['ar_id']) && !$this->gw_this['vars']['tid'])
	{
		$this->str .= $this->oL->m('1348');
		return;
	}
...
else
{
	...
	if (!$isPostError)
	{
		...
		/* Overwrite term parameters */
		if (!isset($arPre['term'][0]['attributes']['uri']) || !$arPre['term'][0]['attributes']['uri'])
		{
			$arPre['term'][0]['attributes']['uri'] = $arPre['term'][0]['value'];
		}
		$arTermParam['term_uri'] = $arPre['term'][0]['attributes']['uri'];
