...
if (!defined('IN_GW'))
{
	die('<!-- $Id: terms_admin.php 410 2008-04-08 22:26:45Z yrtimd $ -->');
}
/* */
class gw_addon_terms_admin extends gw_addon
{
	...
	function get_form_term($vars, $runtime = 0, $ar_broken = array(), $ar_req = array())
	{
		...
		else
		{
			...
			$str_form .= getFormTitleNav( ($vars['is_active'] == 3 ? '' : '<span style="float:right">'.
					$oForm->get_button('submit', 2).
					'<input style="width:24px" class="submitdel" title="'. $this->oL->m('3_remove') . ' ' .
					'" type="submit" value="&#215;" name="remove" ' .
					'onclick="vbcontrol.style.visibility=\'hidden\'" /></span>') . $this->oL->m('term') .
					'<div class="gray xu">'.
					'<a href="'.$this->oHtml->url_normalize($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&w1=viewhistory&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('1294').'</a>'.
					' | <a href="'.$this->oHtml->url_normalize($this->sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&w1=viewkeywords&id='.$this->gw_this['vars']['id'].'&amp;'.GW_TARGET.'='.GW_T_TERMS.'&tid='.$this->gw_this['vars']['tid']).'">'.$this->oL->m('1284').'</a>'.
					' | <a class="ext" href="'.$this->oHtml->url_normalize($this->sys['page_index'].'?'.GW_ACTION.'='.GW_T_TERM.'&d='.$arDictParam['uri'].'&'.GW_TARGET.'='.$arTermParam['uri']).'" onclick="window.open(this.href);return false">'.$this->oL->m('1283').'</a>'.
					'</div>');
		}
...
/*]]>*/</script>';
		...
		$str_form .= '<img style="vertical-align:middle;padding: 0 5px" id="ci-term-dates" src="'.$this->sys['path_img_admin'].'/collapse_off.png" alt="" width="9" height="21" />';
