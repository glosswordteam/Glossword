<?php
/**
 *  $Id: gw_login.php 398 2008-04-06 12:58:30Z yrtimd $
 */
/**
...
function gw_login_form($ar_vars, $ar_broken = array(), $ar_req = array())
{
	...
	{
		$ar_req_m[$k] = isset($ar_req[$k]) ? '&#160;<span class="red"><b>*</b></span>' : '';
		$ar_broken_m[$k] = isset($ar_broken[$k]) ? ' <span class="red"><b>' . $oL->m('reason_9') .'</b></span>' : '';
	}
	$oForm->formwidth = '300';
	...
	else
	{
		...
		$strForm .= '<tbody><tr>'.
				...
				'</tr>';
		$strForm .= '<tr>'.
					'<td class="'.$v_class_1.'">' . $oL->m('password') . ':' . $ar_req_m['user_pass'] . '</td>'.
	...
	else
	{
		...
		if (!empty($arSql))
		{
			...
			if ($arSql['is_active'] == '1')
			{
				...
				/* Add cookies */
				setcookie( $oSess->sid.$sys['token'], $oSess->id_sess, $sys['time_now'] + ($oSess->int_timeout * 2), '/', $sys['server_host'] );
				setcookie( $oSess->sid.'r'.$sys['token'], 1, $sys['time_now'] + ($oSess->int_timeout * 2), '/', $sys['server_host'] );
				/* Close Session class */
				$oSess->sess_close();
				/* redirect to admin page */
				gwtk_header( $sys['server_url'].'/'.$sys['file_admin'] );
			}
			else
			{
				/* Accound disabled */
