...
if (!defined('IN_GW'))
{
	die('<!-- $Id: func.catalog.inc.php 401 2008-04-06 18:46:47Z yrtimd $ -->');
}
/**
	...
	if (sizeof($ar) > 0)
	{
		if ($startId != 0)
		{
			...
			if ($p != 0)
			{
				...
				{
					$image .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['l'] : $arTxtTread['l'];
				}
				else
				{
					$image .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['t'] : $arTxtTread['t'];
				}
			} // if $p != 0;
			...
			if (isset($ar[$startId]['ch']) && is_array($ar[$startId]['ch']))
			{
				...
				if (isset($ar[$p]['img']))
				{
					...
					if($startId == $ar[$p]['max'])
					{
						$ar[$startId]['img'] .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['trans'] : $arTxtTread['trans'];
					}
					else
					{
						$ar[$startId]['img'] .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['i'] : $arTxtTread['i'];
					}
				}
				$image .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['m'] : $arTxtTread['m'];
			}
			else
			{
				if($ar[$startId]['p'] != 0)
				{
					$image .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['c'] : $arTxtTread['c'];
				}
				else
				{
					$image .= ($GLOBALS['topic_mode'] == 'html') ? $arImgTread['space'] : $arTxtTread['space'];
				}
			} // isset($ar[$startId]["ch"])
	...
	if (sizeof($ar) > 0)
	{
		if ($startId != 0)
		{
			...
			if ($topic_mode == 'form')
			{
				...
				// exclusion for Add mode
				if ($gw_this['vars'][GW_ACTION] == GW_ACTION || ($gw_this['vars'][GW_TARGET] == GW_T_DICTS))
				{
					$isBuild = 1;
					// rule for auto-selection in Add mode
					if (isset($ar[$gw_this['vars']['tid']]['p']))
			...
			elseif ($GLOBALS['topic_mode'] == 'html')
			{
				...
				$str .= '</td>';
				$str .= '<td class="xt" style="text-align:center">[';
				$str .= ($isUp && $is_allow_edit) ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&mode=up&tid=' . $ar[$startId]['id'], $oL->m('3_up')) : $oL->m('3_up');
				$str .= '] [';
				$str .= ($isDn && $is_allow_edit) ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&mode=dn&tid=' . $ar[$startId]['id'], $oL->m('3_down')) : $oL->m('3_down');
				$str .= '] [';
				$str .= ($isReset && $is_allow_edit) ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&mode=reset&tid=' . $ar[$startId]['id'], $oL->m('3_reset')) : $oL->m('3_reset');
				$str .= ']</td>';
				$str .= '<td class="xt" style="text-align:center">[';
				$str .= ($is_allow_edit ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_ADD.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&tid='.$ar[$startId]['id'], $oL->m('3_add') ) : $oL->m('3_add') );
				$str .= '] [';
				$str .= ($is_allow_edit ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&tid='.$ar[$startId]['id'], $oL->m('3_edit') ) : $oL->m('3_edit') );
				$str .= '] [';
				$str .= ($is_allow_edit ? $oHtml->a( $page_index . '?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'='.$gw_this['vars'][GW_TARGET].'&remove=1&tid='.$ar[$startId]['id'], $oL->m('3_remove') ) : $oL->m('3_remove') );
				$str .= ']</td>';
				/* 1.8.7: Turn on/off */
				...
				$str .= ($is_allow_edit ? ($ar[$startId]['is_active'] 
							...
							? '<span class="green">'.$oL->m('is_1').'</span>'
							: '<span class="red">'.$oL->m('is_0').'</span>'
						));
				$str .= ']</td>';
				$str .= '</tr>';
			}
