 ...
 * @global  $oL
 */
function gwCreateMessage($user_name, $login_url, $login, $password)
{
	global $sys, $oL;
	/* load language files */
	$oL->getCustom('mail', $sys['locale_name'], 'join');
	...
	$arTrg = array($user_name,
				   $sys['site_name'],
				   $login_url, $oL->m('login'),
				   $login,
				   $oL->m('password'),
				   $password,
				   '<a href="mailto:'. $sys['y_email'].'">'.$sys['y_email'].'</a>'
			 );
	$msgBody = str_replace($arSrc, $arTrg, $oL->m('mail_newuser'));
	$msgSubj = str_replace($arSrc, $arTrg, $oL->m('mail_newuser_subj'));
	/* */
	...
	$oTpl->addVal( 'v:email_signature',  $mail_sig );
	$oTpl->addVal( 'v:message',          $msgBody );
	$oTpl->addVal( 'v:subject',          $msgSubj );
	/* I request you to retain the copyright notice! */
	$oTpl->addVal( 'v:copyright',        $sys['str_branding'] );
	$oTpl->set_tpl(GW_TPL_MAIL);
	$oTpl->parse();
	return $oTpl->output();
}
