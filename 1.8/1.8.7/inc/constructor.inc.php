...
if (!defined('IN_GW'))
{
	die('<!-- $Id: constructor.inc.php 402 2008-04-06 21:18:55Z yrtimd $ -->');
}
/**
...
if ($gw_this['vars']['layout'] != '') // settings for all dictionary pages
{
	...
	if ($gw_this['vars'][GW_ID_DICT])
	{
		...
		{
			$arSqlVkbd = $oDb->sqlExec( $oSqlQ->getQ('get-vkbd-profile', $arDictParam['id_vkbd']), $arDictParam['id'] );
			...
			if (!empty($arSqlVkbd))
			{
				...
				$oTpl->addVal( 'v:keyboard', '<a title="'.$oL->m('virtual_keyboard').'" id="gwkbdcall" onclick="return gwJS.showKbd(\'gw\', ['.
						$arSqlVkbd['vkbd_letters'] . ']);" class="submitok" style="width:2em">'.
						$ar_theme['txt_virtual_keyboard'] . '</a><table style="visibility:hidden" id="gwkbd" cellspacing="0"><tbody><tr><td></td></tr></tbody></table>' );
			}
		}
...
$sys['css_align_numbers'] = 'rtl';
if ($oL->languagelist('1') == 'rtl')
{
	$sys['css_align_numbers'] = 'ltr';
...
switch ($gw_this['vars']['layout'])
{
	...
	case 'index': /* Dictionary title page */
	{
		...
		$p = 0;
		$oTpl->addVal( 'v:nav_pages', getNavToolbar($intSumPages, $p, $sys['page_index'] . '?'.GW_ACTION.'='.GW_A_LIST.'&d='.$arDictParam['uri'].'&p='));
	...
	case GW_A_LIST:
	{
		...
		else
		{
			...
			/* By page */
			$listA = getDictWordList($gw_this['vars']['w1'], $gw_this['vars']['w1'], $gw_this['vars']['w3'], $gw_this['vars'][GW_ID_DICT], $p, 1, $arDictParam['is_show_full']);
			if ($p > 1)
			{
				$gw_this['arTitle'][] = sprintf($oL->m('str_page_of_page'), $oFunc->number_format($p, 0, $oL->languagelist('4')), $oFunc->number_format($listA[3], 0, $oL->languagelist('4')));
				$gw_this['ar_breadcrumb'][] = $oHtml->a($sys['page_index'].
						'?'.GW_ACTION.'='.GW_A_LIST.
						'&'.GW_ID_DICT.'='.$arDictParam['uri'].
						'&'.GW_TARGET.'='.GW_T_DICT.
		...
		if ($intSumPages > 1)
		{
			$oTpl->addVal( 'v:nav_pages', getNavToolbar($intSumPages, $p, $sys['page_index'] . '?'.GW_ACTION.'='.$gw_this['vars']['layout'].'&strict='.$strict.'&d='.$gw_this['vars'][GW_ID_DICT].'&w1='.urlencode($w1).'&w2='.urlencode($w2).'&p='));
		}
	...
	case GW_T_TERM:
	{
		...
		$oTpl->addVal( 'url:term_url',
						$oHtml->a(
							$sys['page_index']. '?a=term&d=' . $gw_this['vars'][GW_ID_DICT] . '&t=' . $listA['tid'],
							$sys['server_proto'] . $sys['server_host'] . $oHtml->url_normalize($sys['page_index']. '?a='.GW_T_TERM.'&d=' . $gw_this['vars'][GW_ID_DICT] . '&t=' . $listA['tid'])
						)
		);
		$oTpl->addVal( 'url:dict_url',
						$oHtml->a(
							$sys['page_index']. '?a=list&d=' . $gw_this['vars'][GW_ID_DICT],
							$sys['server_proto'] . $sys['server_host'] . $oHtml->url_normalize($sys['page_index']. '?a=list&d=' . $gw_this['vars'][GW_ID_DICT])
						)
		);
		// -------------------------------------------------
...
*/
		...
		else
		{
			...
			if ($arDictParam['is_show_term_suggest'])
			{
				...
				$gw_this['url_term_suggest'] = $oHtml->a('#', $oL->m('1095'));
				$oHtml->setTag('a', 'onclick', '');
			...
			if ($arDictParam['is_show_term_report'])
			{
				...
				$gw_this['url_term_report'] = $oHtml->a('#', $oL->m('bug_report'));
				$oHtml->setTag('a', 'onclick', '');
			...
			if ($arDictParam['is_show_add_to_favorites'])
			{
				...
				function gw_addBookmark(url, title) {
					if (agent.indexOf("opera") != -1) { if (window.opera && window.print) { return true; } }
			...
			if ($arDictParam['is_show_add_to_search'])
			{
				...
				$gw_this['url_show_add_to_search'] = $oHtml->a('#', $oL->m('1271'));
				$oHtml->setTag('a', 'onclick', '');
			...
			if ($arDictParam['is_show_printversion'])
			{
				...
				$gw_this['url_show_printversion'] = $oHtml->a('#', $oL->m('printversion'));
				$oHtml->setTag('a', 'onclick', '');
	...
	case GW_A_SEARCH: /* Search mode */
	{
		...
		{
			/* 1st search query */
			gw_search($q, $gw_this['arDictListSrch'], $gw_this['vars']['srch']);
		#	return;
		}
		else
		{
			/* List search results */
			$gw_this['arSrchResults'] = gw_search_results($id_srch, $p, $gw_this['vars'][GW_ID_DICT]);
		...
		if ($intSumPages > 1) // enable page navigation
		{
			$oTpl->addVal( 'l:pages', $oL->m('L_pages').':');
			$oTpl->addVal( 'v:nav_pages',
				getNavToolbar($intSumPages, $p, $sys['page_index'] . '?'.GW_ACTION.'='.$gw_this['vars'][GW_ACTION].'&id_srch='.$id_srch.'&d='.$arDictParam['id'].'&visualtheme='.$gw_this['vars']['visualtheme'].'&p=')
			);
		}
		/* */
...
if (!isset($ar_theme['str_nav_prepend_a'])){ $ar_theme['str_nav_prepend_a'] = ''; }
if (!isset($ar_theme['str_nav_append_a'])){ $ar_theme['str_nav_append_a'] = ''; }
if (!isset($ar_theme['str_nav_class'])){ $ar_theme['str_nav_class'] = ''; }
...
if (!isset($kw) || !is_array($kw)){ $kw = array(); }
	...
	|| ($gw_this['vars'][GW_ACTION] == GW_A_LIST) ))
{
	...
	$oHtml->setTag('a', 'class', 'on');
	$oTpl->addVal( 'url:dict', $oHtml->a($sys['page_index']. '?a=index&d=' . $gw_this['vars'][GW_ID_DICT], strip_tags($arDictParam['title'])));
	$oHtml->setTag('a', 'class', '');

	/* Do not highlight link to main page */
	$oTpl->addVal( 'url:site_name', $oHtml->a($sys['page_index'], strip_tags( $sys['site_name'])));
