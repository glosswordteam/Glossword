...
if (!defined('IN_GW'))
{
	die('<!-- $Id: func.sql.inc.php 415 2008-04-11 17:30:16Z yrtimd $ -->');
}
/**
...
#				prn_r($arKeywordsQ);
	...
	$qT['date_modified'] = $sys['time_now_gmt_unix'] - 60;
	/* 21 jul 2003: Better protection by adding random Next ID number */
	$qT['id']        = ($qT['id'] == '') ? mt_rand($id_w, ($sys['leech_factor'] * 2) + $id_w) : $qT['id'];
	...
	$qT['term_uri']  = ($qT['term_uri'] == '') ? $qT['id'].'-'.$oCase->lc(text_normalize($str_term_filtered)) : $qT['term_uri'];
	...
	/* TODO: need to remove duplicated code */
...
#			$queryA[] = $oSqlQ->getQ('del-term_id', $arDictParam['tablename'], $id_old, $id_dict);
	...
	if (!$isDelete && $isTermExist)
	{
		...
		{
			$ar_matched_terms[]  = $oHtml->a($sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT.'&'.GW_TARGET.'=' . GW_T_TERM.'&id='.$id_dict.'&tid=' . $arV['id'],
...
#       prn_r($arPre);
		...
		$qT['crc32u'] = crc32($qT['id'].$qT['term'].$qT['term_uri'].$qT['term_1'].$qT['term_2'].$qT['term_3'].$qT['defn']);
		/* Add User ID to term */
		...
		/* $arTermParam is global */
		$arTermParam['id'] = $q2['term_id'] = $qT['id'];
		$q2['dict_id'] = $id_dict;
		// -------------------------------------------------
...
#		$v2 = gw_stripslashes($v2);
		...
		for (reset($arFields); list($id_field, $fV) = each($arFields);)
		{
			if (isset($arKeywords[$id_field]) && in_array($v2, $arKeywords[$id_field]))
			{
				$q2['term_match'] = $id_field;
				$q1['word_text'] = $v2;
