...
if (!defined('IN_GW'))
{
	die('<!-- $Id: func.text.inc.php 397 2008-04-06 12:52:25Z yrtimd $ -->');
}
/**
...
function gethtml_metarefresh($url)
{
	...
	if (isset($oSess))
	{
		$url = $oSess->url($oSess->url_admin . $url);
	}
	return '<meta http-equiv="Refresh" content="' . $sys['time_refresh'] . ';url=' . $url . '" />';
}
/* */
...
function getFormTitleNav($title = 'title', $funcnav = '')
{
	...
	$str .= '<tbody><tr style="background:'.$ar_theme['color_4'].'">';
	if ($funcnav != '')
	{
		$str .= '<td style="width:50%;text-align:'.$sys['css_align_left'].';" class="xm">' . $title . '</td>';
		$str .= '<td style="text-align:'.$sys['css_align_right'].';">' . $funcnav . '</td>';
	}
	else
	{
		$str .= '<td style="color:#7F7771;text-align:'.$sys['css_align_left'].';" class="xm">' . $title . '</td>';
	}
	$str .= '</tr>';
	$str .= '</tbody></table>';
	return $str;
...
function gw_regex_url($url)
{
	...
	/* http in front */
	if (!preg_match("/^(http|news|https|ftp|aim|callto):\/\//", $url['html'] ))
	{
	...
	/* Used for title="" also */
	$stripped = preg_replace("/^(http|ftp|https|news|aim|callto):\/\/(\S+)$/i", "\\2", $url['show']);
	/* Chunk long URLs */
	if (strlen($url['show']) > 40 ) { $is_skip = 1; }
	if (!preg_match("/^(http|ftp|https|news|aim|callto):\/\//i", $url['show'])) { $is_skip = 1; }
	$str_show = $url['show'];
	if ($is_skip)
	{
		$uri_type = preg_replace("/^(http|ftp|https|news|aim|callto):\/\/(\S+)$/i", "\\1", $url['show']);
...
#@header("content-type: text/html; charset=utf-8");
	...
	for (; list($k, $v) = each($ar_words);)
	{
		...
		if ($is_both)
		{
			$t = preg_replace('#('.preg_quote($v, '/').')#i', '<strong class="'.$classname.'">\\1</strong>', $t);
		}
		elseif ($is_center)
		{
			$t = preg_replace('#(^|[\x09-\xff])('.preg_quote($v, '/').')([\x09-\xff]|$)#iu', '\\1<strong class="'.$classname.'">\\2</strong>\\3', $t);
		}
		elseif ($is_right)
		{
			$t = preg_replace('#(^|[\x09-\xff])('.preg_quote($v, '/').')#iu', '\\1<strong class="'.$classname.'">\\2</strong>', $t);
		}
		elseif ($is_left)
		{
			$t = preg_replace('#('.preg_quote($v, '/').')([\x09-\xff]|$)#iu', '<strong class="'.$classname.'">\\1</strong>\\2', $t);
		}
		else
		{
			$t = preg_replace('#('.preg_quote($v, '/').')#i', '<strong class="'.$classname.'">\\1</strong>', $t);
