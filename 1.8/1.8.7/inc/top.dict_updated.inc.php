 ...
 */
	...
	for (; list($arK, $arV) = each($arSql);)
	{
		...
		if (GW_IS_BROWSE_ADMIN)
		{
			...
			{
				$strData .= '<td class="nobr">[ ';
				$strData .= $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='.GW_A_EDIT .'&'. GW_TARGET.'='.GW_T_DICTS. '&id='.$arV['id'] . '&tid='.$arV['id'], $oL->m('3_edit'));
				$strData .= ' ] [ ';
					$strData .= $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='. GW_A_ADD .'&'. GW_TARGET.'='.GW_T_TERMS. '&id='.$arV['id'] . '&tid='.$arV['id'], $oL->m('3_add_term'));
				$strData .= ' ]&#160;</td>';
				$strData .= '<td>' . $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='.GW_A_EDIT .'&'. GW_TARGET.'='.GW_T_DICTS. '&id='.$arV['id'] . '&tid='.$arV['id'], $arV['title']) . '</td>';
			}
			else
			{
				$strData .= '<td class="nobr">[ '.$oL->m('3_edit').' ] [ '.$oL->m('3_add_term').' ]&#160;</td>';
				$strData .= '<td>'.$arV['title'].'&#160;</td>';
			}
		}
		else
		{
			...
			$strData .= '<td>' . $oHtml->a($sys['page_index'] . '?' .GW_ACTION. '=list&p=1&d=' . $arV['id'], $arV['title']) . '</td>';
		}
