...
if (defined('IN_GW'))
{
	die("<!-- $Id: gw_admin.php 414 2008-04-10 20:17:36Z yrtimd $ -->");
}
/**
...
$gw_this['vars'] = $oGlobals->register(array(
	...
	GW_ACTION,GW_SID,GW_SID.'r',GW_TARGET,GW_LANG_I,GW_LANG_C,'visualtheme',
...
else
{
		...
		&& $gw_this['vars']['is']['save_'.GW_LANG_I])
	{
		...
		gw_set_cookie('gw_'.GW_LANG_I, $gw_this['vars'][GW_LANG_I], 0);
		gw_set_cookie('gw_is_save_'.GW_LANG_I, 1, 0);
		$gw_this['cookie']['gw_'.GW_LANG_I] = $gw_this['vars'][GW_LANG_I];
		$gw_this['cookie']['gw_is_save_'.GW_LANG_I] = 1;
	}
	else
	{
		gw_set_cookie('gw_'.GW_LANG_I, '', 0);
		gw_set_cookie('gw_is_save_'.GW_LANG_I, '', 0);
...
$ar_menu_info = array();
$ar_menu_info[] = '<span class="green"><strong>' . $oL->m('online') . '</strong></span>: <strong>'.
				$oHtml->a( $sys['page_admin'].'?'.GW_ACTION.'='.GW_A_EDIT. '&t=users', $oSess->user_get('user_fname').' '.$oSess->user_get('user_sname'), $oL->m('3_profile') ) .
				'</strong>';
$ar_menu_info[] = '<span class="black">'.$oFunc->date_SecToTime( $sys['time_now_gmt_unix'] - $oSess->user_get('date_login') ).'</span>';
$ar_menu_info[] = $oHtml->a( $sys['page_admin'].'?a=logout', $oL->m('3_logout') );
...
$oTpl->addVal( 'url:add_term',       $oHtml->a($sys['page_admin'] . '?' .GW_ACTION.'='.GW_A_ADD .'&'. GW_TARGET.'='.GW_T_TERMS, $oL->m('3_add_term')) );
$oTpl->addVal( 'l:term',             $oL->m('term') );
/* Show notice */
if ($gw_this['vars']['note_afterpost'])
{
	$oTpl->addVal( 'v:note_afterpost', '<div class="note-afterpost" id="note-afterpost"><a href="#" onclick="gw_getElementById(\'note-afterpost\').style.display=\'none\';return false" style="font-size:150%;padding:0 5px;display:block;float:right">×</a><script type="text/javascript">/*<![CDATA[*/gwJS.FXfadeOpac(\'note-afterpost\');/*]]>*/</script>'.$gw_this['vars']['note_afterpost'].'</div>' );
}
...
$arTplVars['srch'][] = array('v:chk_srch_in_term_removed' => '' );
...
elseif (intval($gw_this['vars']['srch']['in']) == 100)
{
	/* search in unapproved terms */
	$arTplVars['srch'][] = array('v:chk_srch_in_term_unapproved' => ' checked="checked"' );
}
elseif (intval($gw_this['vars']['srch']['in']) == 101)
{
	/* search in unapproved terms */
	$arTplVars['srch'][] = array('v:chk_srch_in_term_incomplete' => ' checked="checked"' );
}
elseif (intval($gw_this['vars']['srch']['in']) == 102)
{
	/* search in unapproved terms */
	$arTplVars['srch'][] = array('v:chk_srch_in_term_removed' => ' checked="checked"' );
}
else
{
...
if ($gw_this['vars']['d'])
{
	...
	$strDictDetails .= '<tbody><tr class="xt" style="vertical-align:top">';
	/* Link to dictionary */
	$strDictDetails .= '<td><span class="gray">' . $oL->m('dict') . ':</span><br />'. 
						'<a class="ext" href="'.( $sys['page_index'].'?'.GW_ACTION.'=list&amp;d='.$arDictParam['uri'] ).
						'" onclick="window.open(this.href);return false;">' . $arDictParam['title'] . '</a></td>';
	$strDictDetails .= '<td style="width:25%"><span class="gray">' . $oL->m('termsamount') . ':</span><br />'. $oFunc->number_format($arDictParam['int_terms'], 0, $oL->languagelist('4')) .'</td>';
	$strDictDetails .= '<td style="width:10%"><span class="gray">' . $oL->m('status') . ':</span><br />'. $str_is . '</td>';
	$strDictDetails .= '<td style="width:10%"><span class="gray">' . $oL->m('lang') . ':</span><br />'.  ($arDictParam['lang'] ? $languagelist[$arDictParam['lang']] : $languagelist[$sys['locale_name']] ) . '</td>';
	$strDictDetails .= '<td style="width:10%"><span class="gray">' . $oL->m('size') . ':</span><br />'. $oFunc->number_format($arDictParam["int_bytes"]/1024, 1, $oL->languagelist('4')) . '&#160;'. $oL->m('kb') . '</td>';
	$strDictDetails .= '</tr>';
