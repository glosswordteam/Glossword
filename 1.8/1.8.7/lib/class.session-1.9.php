...
class gw_session_1_9
{
	...
	function sess_init($id_sess = 0)
	{
		if ($id_sess)
		{
			...
			if (empty($arSql))
			{
				...
				/* Remove cookie */
				setcookie( $this->sid.$this->sys['token'], $this->id_sess, time() - 2, '/', $this->sys['server_host'] );
	...
	function sess_close()
	{
		...
		$this->ar_sess['date_changed'] = $this->time_now_gmt_unix;
		/* Continue cookie */
		@setcookie( $this->sid.$this->sys['token'], $this->id_sess, $this->time_now + ($this->int_timeout * 2), '/', $this->sys['server_host'] );
...
#prn_r( $this->id_user, 'id_user'  );
	...
	function _user_update()
	{
		...
		unset($this->ar_user['is_remember']);
	...
	function error($error_code = 0)
	{
		...
		setcookie( $this->sid.$this->sys['token'], $this->id_sess, time() - 2, '/', $this->sys['server_host'] );
		setcookie( $this->sid.'r'.$this->sys['token'], 1, $this->sys['time_now'] - 2, '/', $this->sys['server_host'] );
		$msg = '';
		switch ($error_code)
		{
			case 1:
		...
		print '<meta http-equiv="Refresh" content="'.$this->time_refresh.';url='.$this->url($this->url_login).'" />';
		print '</head><body>';
		print '<div><div style="font: 100% sans-serif; margin: 1em">';
		print $msg;
		print '<p><a href="'.$this->url($this->url_login).'">'.$this->url($this->url_login).'</a></p>';
		print '</div></div>';
		print '</body></html>';
		$this->user_close();
		exit;
