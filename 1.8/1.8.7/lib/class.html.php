...
#		prn_r($arUrl);
		...
		$arRule2 = split($str_implode, end($arRule));
		$arUrl = array();
		if (sizeof($arRule2) > 1 )
		{
	...
	function url_rule2str($arP, $arRule)
	{
		...
		$arRule2 = split($str_split, end($arRule));
		$arP2 = split($str_split, str_replace($this->mod_rewrite_suffix, '', end($arP)));
		$int_p = sizeof($arP2);
		while (list($k, $v) = each($arRule2))
		{
			if (($v != '') && isset($arP2[$k]) 
