<?php
/**
 * © 2003-2008 Dmitry N. Shilnikov <dev at glossword dot info>
 * $Id: class.func.php,v 1.20 2006/10/06 12:11:57 yrtimd Exp $
 * http://glossword.info/
 */
/* --------------------------------------------------------
...
function prn_r($ar, $comment = '')
{
	if (is_array($ar) || is_object($ar))
	{
		...
		{
			ksort($ar);
			$ar = htmlspecialchars_ltgt($ar);
		}
		#$ar = array_map("htmlspecialchars_ltgt", $ar);
	}
	elseif (is_string($ar))
	{
		$ar = htmlspecialchars_ltgt($ar);
	}
	print '<pre style="text-align:left;background:#FFF;font-size:0.9em; margin:0">';
	if ($comment != '')
	{
